...
static int mt6360_regulator_enable(struct regulator_dev *rdev)
{
	...
	{
		/*+dunweichao.wt,2024.8.26,sdcard bring,MT6360 platform modifies ldo configuration,0x0 means low level is SD unplugged*/
		#if defined(CONFIG_SD_DET_ACTIVE_HIGH)
		ret = regmap_update_bits(regmap, MT6360_REG_LDO5CTRL0,
					 MT6360_MASK_SDCARD_DET_EN, 0x0);
		#else
		ret = regmap_update_bits(regmap, MT6360_REG_LDO5CTRL0,
					 ...
					 int tbl_size)
{
	...
	for (i = 0; i < tbl_size; i++) {
		...
		ret = devm_request_threaded_irq(&pdev->dev, irq, NULL, irq_desc->handler, 0,
						irq_desc->name, rdev);
		if (ret) {
			dev_dbg(&pdev->dev, "Fail to request %s irq\n", irq_desc->name);
			return ret;
		}
	}
...
static int mt6360_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6360_regulator_descs); i++) {
		...
		if (IS_ERR(rdev)) {
			dev_dbg(&pdev->dev, "Failed to register  %d regulator\n", i);
			return PTR_ERR(rdev);
		}
		...
		if (ret) {
			dev_dbg(&pdev->dev, "Failed to register  %d regulator irqs\n", i);
			return ret;
		}
	}
