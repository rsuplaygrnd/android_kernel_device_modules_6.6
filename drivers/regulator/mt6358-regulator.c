...
static const u32 vio28_voltages[] __maybe_unused = {
	2900000, //modify vio28 voltage to 2.9V
};
...
static struct mt6397_chip *mt6358_chip;
struct regmap *mt6358_pmic_nolock_regmap;
...
{
	int ret = 0;
#if 1//defined(CONFIG_PMIC_HW_ACCESS_EN)
	if (!map) {
					  ...
					  unsigned int SHIFT)
{
	...
	ret = pmic_write_device(mt6358_pmic_nolock_regmap, RegNum, val, MASK, SHIFT);
	return abs(ret);
}

unsigned int mt6358_pmic_config_interface(unsigned int RegNum,
				   unsigned int val,
				   unsigned int MASK,
				   unsigned int SHIFT)
{
	...
	ret = pmic_write_device(mt6358_chip->regmap, RegNum, val, MASK, SHIFT);
	return ret;
}
EXPORT_SYMBOL_GPL(mt6358_pmic_config_interface);
#endif
...
#ifdef CONFIG_MTK_S96818_CAMERA
	mt6358_chip = mt6397;
	mt6358_pmic_nolock_regmap = mt6397->regmap;
#endif
...
#if IS_BUILTIN(CONFIG_DEVICE_MODULES_REGULATOR_MT6358)
