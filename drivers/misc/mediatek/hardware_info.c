...
/* +S96901AA5-740, zhouweijie.wt, 2024.08.19, add read cmdline */
#include <linux/of.h>
#include <asm/setup.h>
/* -S96901AA5-740, zhouweijie.wt, 2024.08.19, add read cmdline */
...
/* +S96901AA5-740, zhouweijie.wt, 2024.08.19, add read cmdline */
char wt_saved_command_line[COMMAND_LINE_SIZE] = {0};
EXPORT_SYMBOL_GPL(wt_saved_command_line);
/* -S96901AA5-740, zhouweijie.wt, 2024.08.19, add read cmdline */
unsigned char n28_lcm_sn[20] = {0};
EXPORT_SYMBOL_GPL(n28_lcm_sn);
...
static void hw_get_cmd(void)
{
	...
	if (of_chosen) {
		...
		if (!bootargs)
			...
			strcpy(wt_saved_command_line, bootargs);
			pr_err("%s: bootargs: %s\n", __func__, wt_saved_command_line);
		}
	} else
...
static char* hwid_get(void)
{
	...
	s1 = strstr(wt_saved_command_line, "hw_id=");
	if(!s1) {
...
static char* hw_lcm_serialnum_get(void)
{
	...
	if ((strcmp(Lcm_name, "n28_td4160_dsi_vdo_hdp_xinxian_inx") == 0)
		...
		s1 = strstr(wt_saved_command_line, "lcm_serialnum:");
		if(!s1) {
...
{
	int ret, i;
	pr_info("%s: hardwareinfo_init_module\n", __func__);
	for(i = 0; i < HARDWARE_MAX_ITEM; i++)
