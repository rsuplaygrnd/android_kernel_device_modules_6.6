...
#include "n28hi5021qreardc_mipiraw_otp.h"

#define PFX "n28hi5021qreardc_camera_otp"
#define LOG_INF(format, args...)    \
     pr_info(PFX "[%s] " format, __func__, ##args)

#define EEPROM_BL24SA64D_ID 0xA0

static kal_uint8 Dc_XGCbuffer[hi5021Q_OTP_XGC_LEN] = {0};
static kal_uint8 Dc_QGCbuffer[hi5021Q_OTP_QGC_LEN] = {0};
static kal_uint8 Dc_PGCbuffer[hi5021Q_OTP_PGC_LEN] = {0};

static kal_uint16 Dc_XGC_GB_setting_burst[XGC_GB_DATA_SIZE];
static kal_uint16 Dc_XGC_GR_setting_burst[XGC_GR_DATA_SIZE];
static kal_uint16 Dc_QBGC_setting_burst[QBGC_DATA_SIZE];
static kal_uint16 Dc_PGC_GB_setting_burst[PGC_GB_DATA_SIZE];
static kal_uint16 Dc_PGC_GR_setting_burst[PGC_GR_DATA_SIZE];

struct stCAM_CAL_DATAINFO_STRUCT n28hi5021qreardc_eeprom_data = {
...
struct stCAM_CAL_CHECKSUM_STRUCT n28hi5021qreardc_Checksum[12] =
{
...
kal_uint16 hi5021q_read_eeprom(kal_uint32 addr)
{
...
static u8* hi5021q_read_eeprom_onboot(int size) {
    ...
    memcpy(ebuf, (void *)&(n28hi5021qreardc_eeprom_data.sensorVendorid), OTP_MAP_VENDOR_HEAD_ID_BYTES);
...
void Dc_apply_sensor_Cali(void)
{
     kal_uint8 *XGC_CalData = (kal_uint8*)Dc_XGCbuffer;
     kal_uint8 *QGC_CalData = (kal_uint8*)Dc_QGCbuffer;
     kal_uint8 *PGC_CalData = (kal_uint8*)Dc_PGCbuffer;
...
#endif
              Dc_XGC_GB_setting_burst[2*length_xgc_gb]=hi5021_xgc_gb_addr;
              Dc_XGC_GB_setting_burst[2*length_xgc_gb+1]=hi5021_xgc_gb_data;
              length_xgc_gb++;
...
//Burst write XGC_Gb line //
          ...
          {
               skhynix_write_burst_mode(Dc_XGC_GB_setting_burst,
                    sizeof(Dc_XGC_GB_setting_burst)/sizeof(kal_uint16));
          }
#if HI5021Q_ISP_CALIBRATION_OTP_DUMP
          pr_info("================hi5021q XGC_GB burst write data.================\n");
          for(n=0; n<960; n=n+2)
          {
               pr_info("hi5021q Xgc_gb addr:0x%x,data:0x%x", Dc_XGC_GB_setting_burst[n], Dc_XGC_GB_setting_burst[n+1]);
               if(n%10 == 9)
               {
                    pr_info("\n");
               }
...
//*********Burst write apply XGC GR line start**************//
     {
...
#endif
               Dc_XGC_GR_setting_burst[2*length_xgc_gr]=hi5021_xgc_gr_addr;
               Dc_XGC_GR_setting_burst[2*length_xgc_gr+1]=hi5021_xgc_gr_data;
               length_xgc_gr++;
          ...
          {
               skhynix_write_burst_mode(Dc_XGC_GR_setting_burst,
                    sizeof(Dc_XGC_GR_setting_burst)/sizeof(kal_uint16));
          }
#if HI5021Q_ISP_CALIBRATION_OTP_DUMP
          pr_info("================hi5021q XGC_GR burst write data.================\n");
          for(n=0; n<960; n=n+2)
          {
               pr_info("hi5021q Xgc_gr addr:0x%x,data:0x%x", Dc_XGC_GR_setting_burst[n], Dc_XGC_GR_setting_burst[n+1]);
               if(n%10 == 9)
               {
                    pr_info("\n");
               }
     ...
     //*********Burst write apply QBGC  start**************//
     {
...
#endif
              Dc_QBGC_setting_burst[2*length_qbgc]=hi5021_qbgc_addr;
              Dc_QBGC_setting_burst[2*length_qbgc+1]=hi5021_qbgc_data;
              length_qbgc++;
              hi5021_qbgc_addr += 2;
          }
          //Burst write QBGC  //
          ...
          {
               skhynix_write_burst_mode(Dc_QBGC_setting_burst,
                    sizeof(Dc_QBGC_setting_burst)/sizeof(kal_uint16));
          }
#if HI5021Q_ISP_CALIBRATION_OTP_DUMP
          pr_info("================hi5021q QBGC burst write data.================\n");
          for(n=0; n<1280; n=n+2)
          {
               pr_info("hi5021q QBGC addr:0x%x,data:0x%x", Dc_QBGC_setting_burst[n], Dc_QBGC_setting_burst[n+1]);
               if(n%10 == 9)
               {
                    pr_info("\n");
               }
     ...
     //*********Burst write apply PGC Gb line start**************//
     {
...
#endif
               Dc_PGC_GB_setting_burst[2*length_pgc_gb]=hi5021_pgc_gb_addr;
               Dc_PGC_GB_setting_burst[2*length_pgc_gb+1]=hi5021_pgc_gb_data;
               length_pgc_gb++;
          ...
          {
               skhynix_write_burst_mode(Dc_PGC_GB_setting_burst,
                    sizeof(Dc_PGC_GB_setting_burst)/sizeof(kal_uint16));
          }
#if HI5021Q_ISP_CALIBRATION_OTP_DUMP
          pr_info("================hi5021q PGC_GB burst write    data.================\n");
          for(n=0; n<946; n=n+2)
          {
               pr_info("hi5021q pgc_gb addr:0x%x,data:0x%x", Dc_PGC_GB_setting_burst[n], Dc_PGC_GB_setting_burst[n+1]);
               if(n%10 == 9)
               {
                    pr_info("\n");
               }
     ...
     //*********Burst write apply PGC GR line start**************//
     {
...
#endif
               Dc_PGC_GR_setting_burst[2*length_pgc_gr]=hi5021_pgc_gr_addr;
               Dc_PGC_GR_setting_burst[2*length_pgc_gr+1]=hi5021_pgc_gr_data;
               length_pgc_gr++;
          ...
          {
               skhynix_write_burst_mode(Dc_PGC_GR_setting_burst,
                    sizeof(Dc_PGC_GR_setting_burst)/sizeof(kal_uint16));
          }
#if HI5021Q_ISP_CALIBRATION_OTP_DUMP
          pr_info("================hi5021q PGC_GR burst write data.================\n");
          for(n=0; n<946; n=n+2)
          {
               pr_info("hi5021q pgc_gr addr:0x%x,data:0x%x ", Dc_PGC_GR_setting_burst[n], Dc_PGC_GR_setting_burst[n+1]);
               if(n%10 == 9)
               {
                    pr_info("\n");
               }
...
void hi5021q_init_sensor_cali_info(void) {
    if (NULL == n28hi5021qreardc_eeprom_data.dataBuffer) {
        n28hi5021qreardc_eeprom_data.dataBuffer = hi5021q_read_eeprom_onboot(n28hi5021qreardc_eeprom_data.dataLength + OTP_MAP_VENDOR_HEAD_ID_BYTES);
    }
    if (eeprom_do_checksum(n28hi5021qreardc_eeprom_data.dataBuffer, n28hi5021qreardc_Checksum) == 0) {
        imgsensor_set_eeprom_data_bywing(&n28hi5021qreardc_eeprom_data,n28hi5021qreardc_Checksum);
    }
    ...
    if (n28hi5021qreardc_eeprom_data.dataBuffer != NULL) {
        memcpy(Dc_XGCbuffer, (kal_uint8*)&n28hi5021qreardc_eeprom_data.dataBuffer[hi5021Q_OTP_XGC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], hi5021Q_OTP_XGC_LEN);
        memcpy(Dc_QGCbuffer, (kal_uint8*)&n28hi5021qreardc_eeprom_data.dataBuffer[hi5021Q_OTP_QGC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], hi5021Q_OTP_QGC_LEN);
        memcpy(Dc_PGCbuffer, (kal_uint8*)&n28hi5021qreardc_eeprom_data.dataBuffer[hi5021Q_OTP_PGC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], hi5021Q_OTP_PGC_LEN);
	}
