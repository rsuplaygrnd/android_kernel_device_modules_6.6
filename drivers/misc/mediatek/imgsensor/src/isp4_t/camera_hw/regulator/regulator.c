...
extern unsigned int mt6358_pmic_config_interface(unsigned int RegNum,
				   unsigned int val,
				   unsigned int MASK,
				   unsigned int SHIFT);
#endif
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	{
		if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_1050) {
			mt6358_pmic_config_interface(0x1eae, 0x405, 0xfff, 0);   //set dvdd voltage 1.05V
		} else if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_1200) {
			mt6358_pmic_config_interface(0x1eae, 0x600, 0xfff, 0);   //set dvdd voltage 1.2V
		} else if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_1100) {
			mt6358_pmic_config_interface(0x1eae, 0x500, 0xfff, 0);   //set dvdd voltage 1.1V
		}
		pr_info("set dvdd, powertype:%d powerId:%d\n",
					pin,
					regulator_voltage[
					pin_state - IMGSENSOR_HW_PIN_STATE_LEVEL_0]);
	}
