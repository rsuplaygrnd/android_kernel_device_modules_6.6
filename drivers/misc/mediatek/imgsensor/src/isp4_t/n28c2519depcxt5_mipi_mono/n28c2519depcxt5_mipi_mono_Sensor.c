...
#include "n28c2519depcxt5_mipi_mono_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = N28C2519DEPCXT5_SENSOR_ID ,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 5;
            if ((*sensor_id == imgsensor_info.sensor_id) && (N28S5KJN1REARDC_SENSOR_ID == main_sensor_id)){
				...
				printk("5 supply dualcam(n28s5kjn1reardc_mipi_raw + n28c2519depcxt5_mipi_mono) sensor id match!!");
                return ERROR_NONE;
            }
            printk("[lj c2519]n28c2519depcxt5_mipi_mono get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
	...
	if ((*sensor_id != imgsensor_info.sensor_id)|| (N28S5KJN1REARDC_SENSOR_ID != main_sensor_id)) {
		...
		printk("Fail:5 supply dualcam(need:n28s5kjn1reardc_mipi_raw + n28c2519depcxt5_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",
