...
#include "n28s5kjn1reardc_mipi_raw_Sensor.h"
#include "n28s5kjn1reardc_mipi_raw_otp.h"

#define PFX "n28s5kjn1reardc_mipi_raw_otp"
...
static kal_uint16 DC_JN1_HW_GGC[JN1_HW_GGC_SIZE];
static kal_uint16 DC_JN1_HW_GGC_setting[JN1_HW_GGC_SIZE*2];

struct stCAM_CAL_DATAINFO_STRUCT n28s5kjn1reardc_eeprom_data = {
    .sensorID= N28S5KJN1REARDC_SENSOR_ID,
    .deviceID = 0x01,
    .dataLength = 0x3183,
...
struct stCAM_CAL_CHECKSUM_STRUCT n28s5kjn1reardc_Checksum[14] =
{
...
kal_uint16 s5k_read_cmos_eeprom_8(kal_uint16 addr)
{
...
static void dc_read_eepromData_HW_GGC(void)
{
    kal_uint16 idx = 0, idx_2 = 0;
    for (idx = 0; idx < (JN1_HW_GGC_SIZE*2); idx = idx + 2) {
        DC_JN1_HW_GGC[idx_2] = ((s5k_read_cmos_eeprom_8(JN1_HW_GGC_START_ADDR + idx) << 8) & 0xff00) | (s5k_read_cmos_eeprom_8(JN1_HW_GGC_START_ADDR + idx + 1) & 0x00ff);
        idx_2++;
    }
    for (idx = 0; idx <JN1_HW_GGC_SIZE; idx++) {
        DC_JN1_HW_GGC_setting[2*idx] = 0x6F12;
        DC_JN1_HW_GGC_setting[2*idx + 1] = DC_JN1_HW_GGC[idx];
    }
    LOG_INF("read_eepromData_HW_GGC");
}

void dc_write_sensor_HW_GGC(void)
{
    s5k_write_cmos_sensor(0x6028, 0x2400);
    s5k_write_cmos_sensor(0x602A, 0x0CFC);
    #if N28_DC_S5KJN1_OTP
    s5k_table_write_cmos_sensor(DC_JN1_HW_GGC_setting,
        sizeof(DC_JN1_HW_GGC_setting) / sizeof(kal_uint16));
    LOG_INF("burst write_sensor_HW_GGC");
    #else
    kal_uint16 idx = 0;
    for (idx = 0; idx <JN1_HW_GGC_SIZE; idx++) {
        s5k_write_cmos_sensor_8(0x6F12, DC_JN1_HW_GGC[idx]);
    }
    #endif
...
kal_uint16 n28s5kjn1reardc_read_eeprom(kal_uint32 addr)
{
     kal_uint16 get_byte = 0;
...
static u8* n28s5kjn1reardc_read_eeprom_onboot(int size) {
    int i = 0;
    u8* ebuf = NULL;
    ebuf = kmalloc(size, GFP_KERNEL);
    memcpy(ebuf, (void *) &(n28s5kjn1reardc_eeprom_data.sensorVendorid), OTP_MAP_VENDOR_HEAD_ID_BYTES);
    for (i = OTP_MAP_VENDOR_HEAD_ID_BYTES; i < size; i++) {
        ebuf[i] = n28s5kjn1reardc_read_eeprom(i - OTP_MAP_VENDOR_HEAD_ID_BYTES);
    }
    return ebuf;
}

void s5kjn1dc_init_sensor_cali_info(void) {
    if (NULL == n28s5kjn1reardc_eeprom_data.dataBuffer) {
        n28s5kjn1reardc_eeprom_data.dataBuffer = n28s5kjn1reardc_read_eeprom_onboot(n28s5kjn1reardc_eeprom_data.dataLength + OTP_MAP_VENDOR_HEAD_ID_BYTES);
    }
    if (eeprom_do_checksum(n28s5kjn1reardc_eeprom_data.dataBuffer, n28s5kjn1reardc_Checksum) == 0) {
        imgsensor_set_eeprom_data_bywing(&n28s5kjn1reardc_eeprom_data, n28s5kjn1reardc_Checksum);
    }
    LOG_INF("get eeprom data success\n");

    #if N28_DC_S5KJN1_OTP
    dc_read_eepromData_HW_GGC();
    memcpy(XTCBuffer, (kal_uint8*)&n28s5kjn1reardc_eeprom_data.dataBuffer[DC_S5KJN1_OTP_XTC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_XTC_LEN);
    memcpy(SENSORXTCBuffer, (kal_uint8*)&n28s5kjn1reardc_eeprom_data.dataBuffer[DC_S5KJN1_OTP_SENSORXTC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_SENSORXTC_LEN);
    memcpy(PDXTCXTCBuffer, (kal_uint8*)&n28s5kjn1reardc_eeprom_data.dataBuffer[DC_S5KJN1_OTP_PDXTC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_PDXTC_LEN);
    memcpy(SWGCCBuffer, (kal_uint8*)&n28s5kjn1reardc_eeprom_data.dataBuffer[DC_S5KJN1_OTP_SWGCC_OFFSET+1+OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_SWGCC_LEN);
    #endif
}
