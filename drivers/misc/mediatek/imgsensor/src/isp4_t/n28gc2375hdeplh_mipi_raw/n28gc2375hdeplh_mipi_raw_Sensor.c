...
#include "n28gc2375hdeplh_mipi_raw_Sensor.h"

#include <linux/hardware_info.h>	//For hardwareinfo

/****************************Modify Following Strings for Debug****************************/
#define PFX "n28gc2375hdeplh_camera_sensor"
#define LOG_1 LOG_INF("N28GC2375HDEPLH,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
kal_bool N28GC2375HDEPLHDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = N28GC2375HDEPLH_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2)) {
		...
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 1x, N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 1.43x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 2x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 2.84x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 3.97x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 5.68x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 8.14x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 11.34x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
	} else {
		write_cmos_sensor(0x20,0x0c);
		write_cmos_sensor(0x22,0x0e);
		write_cmos_sensor(0x26,0x0e);
		...
		write_cmos_sensor(0xb1,temp>>6);
		write_cmos_sensor(0xb2,(temp<<2)&0xfc);
		LOG_INF("N28GC2375HDEPLHMIPI analogic gain 16.23x , N28GC2375HDEPLHMIPI add pregain = %d\n",temp);
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if ((*sensor_id == imgsensor_info.sensor_id) && (N28HI5021QREARTRULY_SENSOR_ID == main_sensor_id)){
				...
				printk("2 supply dualcam(n28hi5021reartruly_mipi_raw + n28gc2375hdeplh_mipi_raw) sensor id match!!");
				return ERROR_NONE;
			}
	...
	if ((*sensor_id != imgsensor_info.sensor_id)|| (N28HI5021QREARTRULY_SENSOR_ID != main_sensor_id)) {
		...
		printk("Fail:2 supply dualcam(need:n28hi5021reartruly_mipi_raw + n28gc2375hdeplh_mipi_raw) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	N28GC2375HDEPLHDuringTestPattern = KAL_FALSE;
...
UINT32 N28GC2375HDEPLH_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}    /*    N28GC2375HDEPLHMIPI_RAW_SensorInit    */
