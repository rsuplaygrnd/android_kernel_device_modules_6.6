...
#if defined(__CONFIG_CAM_SC1300CS_LEAK__)
 ...
 * Profiling
 ******************************************************************************/
//+bugS96901AA5-742,daizheng.wt,ADD,2024/09/07,fix front sc1300mcs sensor current leak
...
#if defined(__CONFIG_CAM_SC1300CS_LEAK__)
		//+bugS96901AA5-742,daizheng.wt,ADD,2024/09/07,fix ANR problems caused by poweron
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		if (psensor_inst->sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN) {
			...
			if (!strcmp(psensor_inst->psensor_list->name, "w2hi1336fronttruly_mipi_raw")) {
...
static int imgsensor_probe(struct platform_device *pplatform_device)
{
    ...
    if(IS_ERR(cam1_pinctrl)){
        ...
        cam1_vcamd_1v2_pin_en0 = pinctrl_lookup_state(cam1_pinctrl, "cam1_pnd0");
        if (IS_ERR(cam1_vcamd_1v2_pin_en0)) {
        ...
        cam1_vcamd_1v1_pin_en1 = pinctrl_lookup_state(cam1_pinctrl, "cam1_pnd1");
        ...
        cam1_vcamd_pin_en0 = pinctrl_lookup_state(cam1_pinctrl, "cam1_ldo_vcamd_0");
        if (IS_ERR(cam1_vcamd_pin_en0)) {
        ...
        cam1_vcamd_pin_en1 = pinctrl_lookup_state(cam1_pinctrl, "cam1_ldo_vcamd_1");
        if (IS_ERR(cam1_vcamd_pin_en1)) {
        ...
        cam1_rst_pin_en0 = pinctrl_lookup_state(cam1_pinctrl, "cam1_rst0");
        if (IS_ERR(cam1_rst_pin_en0)) {
        ...
        cam1_rst_pin_en1 = pinctrl_lookup_state(cam1_pinctrl, "cam1_rst1");
        if (IS_ERR(cam1_rst_pin_en1)) {
