...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
...
#ifdef DUALCAMERA_DIFF
	//+S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
	LOG_INF("cwd getimgsensorid!!!, main_sensor_id:0x%x\n", main_sensor_id);
	//-S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
#endif
        ...
        do {
            *sensor_id = return_sensor_id()+ 2;
            ...
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2HI5022QREARTXD_SENSOR_ID == main_sensor_id)){
				LOG_INF("5 supply dualcam(need:w2hi5022qreartxd_mipi_raw + w2sc202csdeplh2_mipi_mono) sensor id match!!");
#else
            ...
            LOG_INF("I2C 0x%x,Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 5;
    ...
    //S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2HI5022QREARTXD_SENSOR_ID != main_sensor_id)) {
		LOG_INF("5 supply dualcam(need:w2hi5022qreartxd_mipi_raw + w2sc202csdeplh2_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
#else
    if (*sensor_id != imgsensor_info.sensor_id) {
#endif
