...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 2;
            ...
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2HI5022QREARTXD_SENSOR_ID == main_sensor_id)){
				LOG_INF("2 supply dualcam(w2hi5022qreartxd_mipi_raw + w2c2519depcxt2_mipi_mono) sensor id match!!");
#else
    ...
    //S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2HI5022QREARTXD_SENSOR_ID != main_sensor_id)) {
		LOG_INF("2 supply dualcam(need:w2hi5022qreartxd_mipi_raw + w2c2519depcxt2_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
#else
    if (*sensor_id != imgsensor_info.sensor_id) {
#endif
