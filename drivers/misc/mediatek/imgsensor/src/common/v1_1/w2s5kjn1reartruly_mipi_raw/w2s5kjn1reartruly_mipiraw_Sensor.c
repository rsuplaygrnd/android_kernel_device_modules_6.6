...
#include "w2s5kjn1reartruly_mipiraw_otp.h"
#include "w2s5kjn1reartruly_mipiraw_Sensor_setting.h"
...
kal_uint16 s5ktruly_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
void s5kturly_write_cmos_sensor(kal_uint16 addr, kal_uint16 para)
{
...
{
    s5kturly_write_cmos_sensor(0x0340, imgsensor.frame_length);
    s5kturly_write_cmos_sensor(0x0342, imgsensor.line_length);
...
static void write_shutter(kal_uint32 shutter)
{
    ...
    if (shutter >= 0xFFF0) {  // need to modify line_length & PCLK
     ...
     //   LOG_INF("CintR =%d \n", CintR);
        ...
        s5kturly_write_cmos_sensor(0x0340, Time_Farme & 0xFFFF);  // Framelength
        s5kturly_write_cmos_sensor(0x0202, CintR & 0xFFFF);  //shutter
        s5kturly_write_cmos_sensor(0x0702, 0x0700);
        s5kturly_write_cmos_sensor(0x0704, 0x0700);
    } else {
        if (bNeedSetNormalMode) {
            LOG_INF("exit long shutter\n");
            s5kturly_write_cmos_sensor(0x0702, 0x0000);
            s5kturly_write_cmos_sensor(0x0704, 0x0000);
            bNeedSetNormalMode = KAL_FALSE;
        }

        s5kturly_write_cmos_sensor(0x0340, imgsensor.frame_length);
        s5kturly_write_cmos_sensor(0x0202, imgsensor.shutter);
    }
                     ...
                     kal_bool auto_extend_en)
{
    ...
    if (shutter >= 0xFFF0) {
      ...
      // LOG_INF("CintR = %d, Time_Farme = %d\n", CintR,Time_Farme);
        ...
        s5kturly_write_cmos_sensor(0x0340, Time_Farme & 0xFFFF);
        s5kturly_write_cmos_sensor(0x0202, CintR & 0xFFFF);
        s5kturly_write_cmos_sensor(0x0702, 0x0600);
        s5kturly_write_cmos_sensor(0x0704, 0x0600);
    } else {
        if (bNeedSetNormalMode) {
            LOG_INF("exit long shutter\n");
            s5kturly_write_cmos_sensor(0x0702, 0x0000);
        ...
        kal_uint8 index, kal_uint16 *regDa, MUINT32 regNum)
{
    ...
    for (i = 0; i < regNum; i++)
        s5kturly_write_cmos_sensor(startAddr[index] + 2*i, regDa[i]);
...
static void sensor_init(void)
{
    ...
    s5kturly_write_cmos_sensor(0x6028, 0x4000);
    s5kturly_write_cmos_sensor(0x0000, 0x0003);
    s5kturly_write_cmos_sensor(0x0000, 0x38E1);
    s5kturly_write_cmos_sensor(0x001E, 0x0007);
    s5kturly_write_cmos_sensor(0x6028, 0x4000);
    s5kturly_write_cmos_sensor(0x6010, 0x0001);
    mdelay(5);  //delay 5ms
    s5kturly_write_cmos_sensor(0x6226, 0x0001);
    mdelay(10);  //delay 10ms
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_init_setting,
...
static void preview_setting(void)
{
    ...
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_preview_setting,
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_capture_setting,
...
static void normal_video_setting(kal_uint16 currefps)
{
    ...
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_normal_video_setting,
...
static void hs_video_setting(void)
{
    ...
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_hs_video_setting,
...
static void slim_video_setting(void)
{
    ...
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_slim_video_setting,
...
static void custom1_setting(void)
{
    ...
    /*************MIPI output setting************/
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_custom1_setting,
...
static void custom2_setting(void)
{
    ...
    /*************MIPI output setting************/
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_custom2_setting,
...
static void custom3_setting(void)
{
    ...
    /*************MIPI output setting************/
    s5ktruly_table_write_cmos_sensor(w2s5kjn1reartruly03_custom3_setting,
        ...
        do {
            *sensor_id = ((read_cmos_sensor_8(0x0000) << 8)
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
				...
				int ret;
				ret = s5ktruly_init_sensor_cali_info();
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    LOG_INF("enable: %d\n", enable);
    if (enable) {
        s5kturly_write_cmos_sensor(0x0600, 0x0001);
        s5kturly_write_cmos_sensor(0x0602, 0x0000);
        s5kturly_write_cmos_sensor(0x0604, 0x0000);
        s5kturly_write_cmos_sensor(0x0606, 0x0000);
        s5kturly_write_cmos_sensor(0x0608, 0x0000);
    } else {
        s5kturly_write_cmos_sensor(0x0600, 0x0000);
    }
    ...
    case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
        {
            ...
            if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
                s5k_read_4cell_from_eeprom(data, size);
            }
            break;
        }
