...
struct stCAM_CAL_DATAINFO_STRUCT w2s5kjn1reartruly_eeprom_data ={
...
kal_uint16 s5ktruly_read_cmos_eeprom_8(kal_uint16 addr)
{
...
void s5ktruly_write_sensor_HW_GGC(void)
{
    s5kturly_write_cmos_sensor(0x6028,0x2400);
    s5kturly_write_cmos_sensor(0x602A,0x0CFC);
    #if W2_TXD_S5KJN1_OTP
    s5ktruly_table_write_cmos_sensor(JN1_HW_GGC_setting,
        sizeof(JN1_HW_GGC_setting)/sizeof(kal_uint16));
    pr_info("burst s5ktruly_write_sensor_HW_GGC");
    #else
    kal_uint16 idx = 0;
    ...
    #endif
    pr_info("s5ktruly_write_sensor_HW_GGC");
}
/*w2 2022/04/18, add for HW_GGC write to sensor end*/
void s5ktruly_read_eepromData_HW_GGC(void)
{
    kal_uint16 idx = 0,idx_2 = 0;
    for (idx = 0; idx <(JN1_HW_GGC_SIZE*2);idx = idx + 2) {
        JN1_HW_GGC[idx_2] = ((s5ktruly_read_cmos_eeprom_8(JN1_HW_GGC_START_ADDR + idx)<<8)&0xff00) | (s5ktruly_read_cmos_eeprom_8(JN1_HW_GGC_START_ADDR + idx + 1)&0x00ff);
        pr_info("JN1_HW_GGC[%d] = %d " , idx_2,JN1_HW_GGC[idx_2]);
        idx_2++;
    }
    for (idx = 0; idx <JN1_HW_GGC_SIZE; idx++) {
        JN1_HW_GGC_setting[2*idx] = 0x6F12;
        JN1_HW_GGC_setting[2*idx + 1] = JN1_HW_GGC[idx];
        pr_info("JN1_HW_GGC_setting[%d] = %d " , (2*idx + 1),JN1_HW_GGC_setting[2*idx + 1]);
    }
    pr_info("s5ktruly_read_eepromData_HW_GGC");
}

void s5k_read_4cell_from_eeprom(char *data, kal_uint16 datasize)
{
...
static u8* s5kturly_read_eeprom_onboot(int size) {
	int i = 0;
	u8* ebuf = NULL;
	ebuf = kmalloc(size, GFP_KERNEL);
	memcpy(ebuf, (void *)&(w2s5kjn1reartruly_eeprom_data.sensorVendorid), OTP_MAP_VENDOR_HEAD_ID_BYTES);
	for (i = OTP_MAP_VENDOR_HEAD_ID_BYTES; i < size; i++) {
			ebuf[i] = s5ktruly_read_cmos_eeprom_8(i - OTP_MAP_VENDOR_HEAD_ID_BYTES);
	}
	return ebuf;
}
...
int s5ktruly_init_sensor_cali_info(void)
{
  //// add maincamera s5kjn1 otp code
             if (NULL == w2s5kjn1reartruly_eeprom_data.dataBuffer)
                 w2s5kjn1reartruly_eeprom_data.dataBuffer = s5kturly_read_eeprom_onboot(w2s5kjn1reartruly_eeprom_data.dataLength + OTP_MAP_VENDOR_HEAD_ID_BYTES);
             if (eeprom_do_checksum(w2s5kjn1reartruly_eeprom_data.dataBuffer, w2s5kjn1reartruly_Checksum) == 0)
                 imgsensor_set_eeprom_data_bywing(&w2s5kjn1reartruly_eeprom_data,w2s5kjn1reartruly_Checksum);
             if((w2s5kjn1reartruly_eeprom_data.sensorVendorid >> 24) != w2s5kjn1reartruly_read_eeprom(0x0001)) {
                 ...
                 s5ktruly_read_eepromData_HW_GGC();
                 memcpy(XTCBuffer,       (kal_uint8*)&w2s5kjn1reartruly_eeprom_data.dataBuffer[TRULY_S5KJN1_OTP_XTC_OFFSET       +1 + OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_XTC_LEN);
                 memcpy(SENSORXTCBuffer, (kal_uint8*)&w2s5kjn1reartruly_eeprom_data.dataBuffer[TRULY_S5KJN1_OTP_SENSORXTC_OFFSET +1 + OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_SENSORXTC_LEN);
                 memcpy(PDXTCXTCBuffer,  (kal_uint8*)&w2s5kjn1reartruly_eeprom_data.dataBuffer[TRULY_S5KJN1_OTP_PDXTC_OFFSET     +1 + OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_PDXTC_LEN);
                 memcpy(SWGCCBuffer,     (kal_uint8*)&w2s5kjn1reartruly_eeprom_data.dataBuffer[TRULY_S5KJN1_OTP_SWGCC_OFFSET     +1 + OTP_MAP_VENDOR_HEAD_ID_BYTES], S5KJN1_OTP_SWGCC_LEN);
             }
             return ERROR_NONE;
