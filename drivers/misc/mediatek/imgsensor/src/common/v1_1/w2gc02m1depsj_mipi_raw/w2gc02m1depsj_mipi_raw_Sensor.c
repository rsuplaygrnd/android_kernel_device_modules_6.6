 ...
 *============================================================================
 */
#define PFX "w2gc02m1depsj_camera_sensor"
...
#include "w2gc02m1depsj_mipi_raw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = W2GC02M1DEPSJ_SENSOR_ID,
...
kal_uint16 addr_data_pair_init_gc02m1[] = {
	...
	0xfc, 0x01,
	0xf4, 0x41,
	0xf5, 0xc0, // c0->e3 For YH&YJ&YK&YC
	0xf6, 0x44,
	0xf8, 0x3a,
	0xf9, 0x82,
	0xfa, 0x00,
	...
	0x39, 0x07,
	0x43, 0x04,
	0x46, 0x2a,  // 2a->4a For YH&YJ&YK&YC
	0x7c, 0xa0,
	0xd0, 0xbe,
	0xd1, 0x60,
	0xd2, 0x40,
	...
	/*ISP*/
	0xfe, 0x01,
	0x53, 0x44,  // 44->54 For YH&YJ&YK&YC
	0x87, 0x53,
	0x89, 0x03,
	/*Gain*/
	0xfe, 0x00,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xf5, 0xc0); // c0->e3 For YH&YJ&YK&YC
	write_cmos_sensor(0xf6, 0x44);
	write_cmos_sensor(0xf8, 0x3a);
	...
	write_cmos_sensor(0x08, 0x3c);
	write_cmos_sensor(0x9d, 0x3c);
	...
	/*analog Register width*/
	write_cmos_sensor(0x21, 0x3e);
	write_cmos_sensor(0x29, 0x42);
	write_cmos_sensor(0x44, 0x21);
	write_cmos_sensor(0x4b, 0x11);
	write_cmos_sensor(0x55, 0x1b);
	...
	write_cmos_sensor(0x46, 0x2a);  // 2a->4a For YH&YJ&YK&YC
	...
	/*ISP*/
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0x53, 0x44);  // 44->54 For YH&YJ&YK&YC
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
...
#ifdef DUALCAMERA_DIFF
	//+S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
	LOG_INF("w2gc02m1depsj_mipi_raw getimgsensorid!!! main_sensor_id:0x%x\n", main_sensor_id);
	//-S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
#endif
...
#ifdef DUALCAMERA_DIFF
            //S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2S5KJN1REARST_SENSOR_ID == main_sensor_id)){
				LOG_INF("3 supply dualcam(w2s5kjn1rearst_mipi_raw + w2gc02m1depsj_mipi_raw) sensor id match!!");
#else
    ...
    //S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2S5KJN1REARST_SENSOR_ID != main_sensor_id)) {
		LOG_INF("3 supply dualcam(need:w2s5kjn1rearst_mipi_raw + w2gc02m1depsj_mipi_raw) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
#else
    if (*sensor_id != imgsensor_info.sensor_id) {
#endif
...
UINT32 W2GC02M1DEPSJ_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
