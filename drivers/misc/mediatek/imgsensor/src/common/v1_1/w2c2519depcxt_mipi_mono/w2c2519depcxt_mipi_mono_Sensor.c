 ...
 * Filename:
 * ---------
 *     w2c2519depcxt_mipi_mono_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#define PFX "w2c2519depcxt_mipi_mono"
#define LOG_1 LOG_INF("W2C2519DEPCXT, MIPI 1LANE\n")
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
...
#ifdef DUALCAMERA_DIFF
	//+S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
	LOG_INF("w2c2519depcxt2_mipi_mono getimgsensorid!!!, main_sensor_id:0x%x\n", main_sensor_id);
	//-S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
#endif
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 1;
            ...
            if ((*sensor_id == imgsensor_info.sensor_id) && (W2S5KJN1REARTRULY_SENSOR_ID == main_sensor_id)){
				LOG_INF("4 supply dualcam(w2s5kjn1reartruly_mipi_raw + w2c2519depcxt_mipi_mono) sensor id match!!");
#else
    ...
    //S96901aa5-742,huangmiao.wt,ADD,2024/09/04, distinguish depth camera params for dualcam
    if ((*sensor_id != imgsensor_info.sensor_id) || (W2S5KJN1REARTRULY_SENSOR_ID != main_sensor_id)) {
		LOG_INF("4 supply dualcam(need:w2s5kjn1reartruly_mipi_raw + w2c2519depcxt_mipi_mono) sensor id not match!!,main_sensor_id:0x%x,depth sensor id:0x%x\n",main_sensor_id,*sensor_id);
#else
    if (*sensor_id != imgsensor_info.sensor_id) {
#endif
