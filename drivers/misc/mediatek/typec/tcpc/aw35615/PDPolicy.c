...
{
	AW_U32 sido = 0x80000005;

	sido = sido | ((PD_Power_as_Source / 1000) << 8);
	sido = sido | ((PD_Power_as_Source / 1000) << 16);
...
void policyBISTCarrierMode2(Port_t *port)
{
	...
	switch (port->PolicySubIndex) {
	case 0:
		if ((port->chip_id == AW35615P_CHIP_ID) || (port->chip_id == AW35615F_CHIP_ID)) {
