...
extern void set_eeprom_read_device_id_bywing(u32 sensorId, u32 deviceId);
#endif
...
/********************************************************************
 * N28EEPROM_set_i2c_bus()
 * To i2c client and slave id
 ********************************************************************/

int N28EEPROM_set_i2c_bus(unsigned int deviceID,
			      struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
    return EEPROM_set_i2c_bus(deviceID, cmdInfo);
}
...
/*************************************************
 * N28EEPROM_get_cmd_info function
 *************************************************/

int N28EEPROM_get_cmd_info(unsigned int sensorID,
	struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
    return EEPROM_get_cmd_info(sensorID, cmdInfo);
}

struct stCAM_CAL_CMD_INFO_STRUCT *N28EEPROM_get_cmd_info_ex
	(unsigned int sensorID, unsigned int deviceID)
{
    return EEPROM_get_cmd_info_ex(sensorID, deviceID);
}
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		#if defined(__CONFIG_WING_CAM_CAL__)
		set_eeprom_read_device_id_bywing(ptempbuf->sensorID, ptempbuf->deviceID);
		#endif
