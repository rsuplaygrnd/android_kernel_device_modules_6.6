...
static char* boardid_get(void)
{
	...
	s1 = strstr(wt_saved_command_line, "board_id=");
	if(!s1) {
		LOG_ERR("board_id not found in cmdline\n");
		return s2;
	}
	s1 += strlen("board_id=");
	...
	board_id[10]='\0';
	s1 = board_id;
	LOG_INF("board_id found in cmdline : %s\n", board_id);
...
int camera_switch = 0;
EXPORT_SYMBOL_GPL(camera_switch);
int hs_video_flag =0;
EXPORT_SYMBOL_GPL(hs_video_flag);
int pwm_flag = 0;
/* define usage count */
...
static int ocp8111_pinctrl_set(int pin, int state)
{
	...
	case OCP8111_PINCTRL_PIN_XXX:
		if (state == OCP8111_PINCTRL_PINSTATE_LOW &&
			...
			LOG_INF("flashlights_aw3641 enter torch mode, light_pwm_value= %d\n", light_pwm_value);
			...
			LOG_INF("flashlights_aw3641_PVT enter torch mode, light_pwm_value= %d, flashlight_pwm_en\n", light_pwm_value);
			mt_flashlight_led_set_pwm(0, light_pwm_value, PWM_CLK_OLD_MODE_32K, CLK_DIV128, 100*20);
			ret = pinctrl_select_state(ocp8111_pinctrl, ocp8111_main_low);
			ret = pinctrl_select_state(ocp8111_pinctrl, ocp8111_xxx_high);
			mdelay(6);
			...
			LOG_INF("flashlights_ocp8111_PVT enter torch mode, light_pwm_value= %d, flashlight_pwm_en\n", light_pwm_value);
			mt_flashlight_led_set_pwm(0, light_pwm_value, PWM_CLK_OLD_MODE_32K, CLK_DIV128, 100*20);
			ret = pinctrl_select_state(ocp8111_pinctrl, ocp8111_main_low);
			ret = pinctrl_select_state(ocp8111_pinctrl, ocp8111_xxx_high);
			mdelay(6);
	...
	case OCP8111_PINCTRL_PIN_XXX:
		if (state == OCP8111_PINCTRL_PINSTATE_LOW &&
			...
			udelay(500);
				...
				!IS_ERR(ocp8111_xxx_high) && !IS_ERR(flashlight_pwm_en) && (flash_distinguishes() == USE_OCP8111) && TorchFlag == 1){	//torch mode
			LOG_INF("flashlights_ocp8111 enter video mode, light_pwm_value= %d\n", light_pwm_value);
			...
			ret = pinctrl_select_state(ocp8111_pinctrl, flashlight_pwm_en); //ENM
				...
				!IS_ERR(ocp8111_xxx_high) && !IS_ERR(flashlight_pwm_aw_en) && (flash_distinguishes() == USE_AW3641) && TorchFlag == 1){	//torch mode
			LOG_INF("flashlights_aw3641 enter video mode, light_pwm_value= %d\n", light_pwm_value);
			...
			ret = pinctrl_select_state(ocp8111_pinctrl, flashlight_pwm_aw_en); //ENM
				...
				!IS_ERR(ocp8111_xxx_high) && !IS_ERR(flashlight_pwm_aw_en) && (flash_distinguishes() == USE_AW3641_PVT) && TorchFlag == 1){	//torch mode
			LOG_INF("flashlights_aw3641_PVT enter video mode, light_pwm_value= %d, flashlight_pwm_en\n", light_pwm_value);
			...
			ret = pinctrl_select_state(ocp8111_pinctrl, flashlight_pwm_aw_en); //ENM
				...
				!IS_ERR(ocp8111_xxx_high) && !IS_ERR(flashlight_pwm_aw_en) && (flash_distinguishes() == USE_OCP8111_PVT) && TorchFlag == 1){	//torch mode
			LOG_INF("flashlights_ocp8111_PVT enter video mode, light_pwm_value= %d, flashlight_pwm_en\n", light_pwm_value);
			mt_flashlight_led_set_pwm(0, light_pwm_value, PWM_CLK_OLD_MODE_32K, CLK_DIV128, 100);
			ret = pinctrl_select_state(ocp8111_pinctrl, ocp8111_main_low);
			ret = pinctrl_select_state(ocp8111_pinctrl, ocp8111_xxx_high);
			mdelay(6);
	...
	if (mainFlashFlag) {
		...
		if ((g_torchWaitLock == 0 || hs_video_flag) && camera_switch == 1) {
			...
			if (flash_distinguishes() == USE_AW3641) {
				light_pwm_value = 0;
				LOG_INF("flashlights_aw3641 set torch mode light_pwm_value = %d\n", light_pwm_value);
			}
			else if (flash_distinguishes() == USE_OCP8111){
				light_pwm_value = 70;
				LOG_INF("flashlights_ocp8111 set torch mode light_pwm_value = %d\n", light_pwm_value);
			}
			else if (flash_distinguishes() == USE_OCP8111_PVT){
				light_pwm_value = 70;
				LOG_INF("flashlights_ocp8111_pvt set torch mode light_pwm_value = %d\n", light_pwm_value);
			}
	...
	switch(level){
		case 1:
			if (flash_distinguishes() == USE_AW3641) {
				...
				mt_flashlight_led_set_pwm(2, light_pwm_value / 20, PWM_CLK_OLD_MODE_BLOCK, CLK_DIV32, 100);
				LOG_INF("flashlights_aw3641 set SS torch mode light_pwm_value = %d\n", light_pwm_value);
			}
			else if (flash_distinguishes() == USE_OCP8111) {
				light_pwm_value = 50*20;
				mt_flashlight_led_set_pwm(0, light_pwm_value, PWM_CLK_OLD_MODE_32K, CLK_DIV128, 100*20);
				...
				mt_flashlight_led_set_pwm(0, light_pwm_value, PWM_CLK_OLD_MODE_32K, CLK_DIV128, 100*20);
				LOG_INF("PVT_flashlights_aw3641 set SS torch mode light_pwm_value = %d\n", light_pwm_value);
			}
			else if (flash_distinguishes() == USE_OCP8111_PVT) {
				light_pwm_value = 25*20;
				mt_flashlight_led_set_pwm(0, light_pwm_value, PWM_CLK_OLD_MODE_32K, CLK_DIV128, 100*20);
				LOG_INF("PVT_flashlights_ocp8111 set SS torch mode light_pwm_value = %d\n", light_pwm_value);
			}
...
static struct flashlight_operations ocp8111_ops = {
	.flashlight_open = ocp8111_open,
	.flashlight_release = ocp8111_release,
	.flashlight_ioctl = ocp8111_ioctl,
	.flashlight_strobe_store = ocp8111_strobe_store,
	.flashlight_set_driver = ocp8111_set_driver
};
...
static ssize_t store_flashduty1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	LOG_INF("ss-torch:set torch level,flashduty1= %lu\n", flashduty1);
	switch(flashduty1){
		case 1001:
			if (flash_distinguishes() == USE_AW3641) {
				light_pwm_value = 2*20;
...
static int ss_flashlight_node_create(void)
{
	...
	if (alloc_chrdev_region(&wt_flashlight_devno, 0, 1, WT_FLASHLIGHT_DEVNAME)) {
		...
		pr_err("[flashlight_probe] major: %d, minor: %d ~",
				MAJOR(wt_flashlight_devno),
	...
	wt_flashlight_class = class_create("camera");   //  /sys/class/camera
