...
static int lm3643_cooling_set_cur_state(int channel, unsigned long state)
{
	...
	if (channel < 0 || channel >= LM3643_LED_MAX) {
	...
	if (state == 0) {
		flash->need_cooler = 0;
		flash->target_current[channel] = LM3643_FLASH_BRT_MAX;
		ret = lm3643_torch_brt_ctrl(flash, channel,
