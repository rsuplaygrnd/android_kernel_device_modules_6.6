...
#if IS_ENABLED(CONFIG_MTK_PRIO_TASK_CONTROL)
	if (((sysctl_prio_control_low && (p->prio >= sysctl_prio_control_low)) ||
		(sysctl_prio_control_high && (p->prio <= sysctl_prio_control_high) && !rt_task(p)))  &&	
...
#if IS_ENABLED(CONFIG_MTK_PRIO_TASK_CONTROL)
		if (((sysctl_prio_control_low && (p->prio >= sysctl_prio_control_low)) ||
			(sysctl_prio_control_high && (p->prio <= sysctl_prio_control_high))) &&
			!cpumask_test_cpu(dst_cpu, &effective_softmask))
			continue;
#endif
...
#if IS_ENABLED(CONFIG_MTK_PRIO_TASK_CONTROL)
		 !(latency_sensitive && !cpumask_test_cpu(this_cpu, &effective_softmask)) &&
		 (((sysctl_prio_control_low && (p->prio >= sysctl_prio_control_low)) ||
		 (sysctl_prio_control_high && (p->prio <= sysctl_prio_control_high))) &&
