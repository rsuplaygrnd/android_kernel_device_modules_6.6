...
*****************************************************************************/
#define  CTP_OPEN_SHORT_TEST_INI_NAME	"focaltech_ft8057s_mdt.ini"
#define  CTP_OPEN_SHORT_NODE_NAME	"ctp_openshort_test"
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    pos = 0;
    write_ret = kernel_write(pfile, data_buf, len, &pos);
    if (write_ret <= 0) {
        FTS_TEST_ERROR("kernel_write fail  ret=%d",  write_ret);
    }
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    ret = 0;
    if (fts_ftest->result == TEST_RESULT_FAIL) {
        ret = -ENOMEM;
    }
test_err:
    fts_test_main_exit();
...
static int openshort_test_show(struct seq_file *s, void *v) {
    int ret = 0;
    int result = -1;
    char fwname[FILE_NAME_LENGTH] = { 0 };
    ...
    memset(fwname, 0, sizeof(fwname));
    //snprintf(fwname, FILE_NAME_LENGTH, "%s", buf);
    sprintf(fwname, "%s", CTP_OPEN_SHORT_TEST_INI_NAME);
   ...
   FTS_INFO("tp_openshort_result = %d", result);
   if(result == 0)
        seq_printf(s, "%s\n", "result=1");
    else
        seq_printf(s, "%s\n", "result=0");
...
{
    return single_open(file, openshort_test_show, NULL);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops ctp_openshort_fops = {
...
#else
static const struct file_operations ctp_openshort_fops = {
...
{
    int ret = 0;
    struct proc_dir_entry *openshort_proc = NULL;
    FTS_TEST_FUNC_ENTER();
    ...
    openshort_proc = proc_create(CTP_OPEN_SHORT_NODE_NAME, 0777, ctp_device_proc, &ctp_openshort_fops);
    if (NULL == openshort_proc) {
        FTS_ERROR("create openshort_proc entry fail");
    }
