...
#define GTP_DEBUG(fmt, arg...)				\
	do {									\
		if (gcore_report_log_cfg>=1)						\
			pr_err("<GTP-DBG>[%s:%d]"fmt"\n", __func__, __LINE__, ##arg);\
	} while (0)

#define GTP_INFO(fmt, arg...)				\
	do {									\
		if (gcore_report_log_cfg>=2)						\
			pr_err("<GTP-DBG>[%s:%d]"fmt"\n", __func__, __LINE__, ##arg);\
	} while (0)

#define GTP_REPORT(fmt, arg...)				\
	do {									\
		if (gcore_report_log_cfg>=3)						\
...
extern	struct workqueue_struct *charger_notify_wq_gc;
extern	struct work_struct gc_resume_work;
 ...
 * Declaration
 */
extern void gc_resume_workqueue_callback(struct work_struct *work);
...
extern void call_proximity_notifier(void);
#endif
