...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
	...
	ret = request_firmware(&fw, MP_TEST_INI, dev);
	if (ret == 0) {
		GTP_DEBUG("firmware request(%s) success", MP_TEST_INI);
		buff = kzalloc(fw->size + 1, GFP_KERNEL);
		GTP_DEBUG("firmware size is:%zu", fw->size + 1);
		if (!buff) {
		...
		GTP_ERROR("firmware request(%s) fail,ret=%d", MP_TEST_INI, ret);
}
...
static const struct proc_ops  gc_proc_ctp_openshort_test_fops = {
