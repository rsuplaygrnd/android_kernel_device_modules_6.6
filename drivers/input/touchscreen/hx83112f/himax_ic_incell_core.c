...
static void himax_mcu_sense_on(uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		if (retry >= 5) {
			...
			if (himax_bus_write(pic_op->adr_i2c_psw_lb[0],
				NULL,
				tmp_data,
				1) < 0) {
			...
			if (himax_bus_write(pic_op->adr_i2c_psw_ub[0],
				NULL,
				tmp_data,
				1) < 0) {
				E("%s: cmd=%x bus access fail!\n",
...
static uint32_t himax_mcu_check_CRC(uint8_t *start_addr, int reload_length)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_reload_addr_cmd_beat,
							tmp_data,
							DATA_LEN_4);
...
static int himax_mcu_ulpm_in(void)
{
	...
	do {
		...
		if (himax_bus_write(pfw_op->addr_ulpm_34[0],
			NULL,
			pfw_op->data_ulpm_11,
			1) < 0) {
			I("%s: spi write fail!\n", __func__);
		...
		if (himax_bus_read(pfw_op->addr_ulpm_34[0],
			tmp_data,
			1) < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
	...
	do {
		...
		if (himax_bus_write(pfw_op->addr_ulpm_33[0],
			NULL,
			pfw_op->data_ulpm_33,
			1) < 0) {
			I("%s: spi write fail!\n", __func__);
		...
		if (himax_bus_read(pfw_op->addr_ulpm_33[0],
			tmp_data, 1) < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
...
static void himax_mcu_usb_detect_set(const uint8_t *cable_config)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
...
static void himax_mcu_read_FW_ver(void)
{
	...
	if( t_lcm_name == 3)
		sprintf(Ctp_name,"DSBJ,HX83112F,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
	else if ( t_lcm_name == 4)
		sprintf(Ctp_name,"DJN,HX83112F,FW:0x%02x", ic_data->vendor_touch_cfg_ver);
}
...
static bool himax_mcu_read_event_stack(uint8_t *buf, uint32_t length)
{
	...
	if (private_ts->debug_log_level & BIT(2)) {
		...
		private_ts->bus_speed = (len * 9 * 1000000
...
static bool hx_bin_desc_data_get(uint32_t addr, const uint8_t *flash_buf)
{
...
static void himax_resume_proc(bool suspended)
{
	...
	if (g_core_fp.fp_0f_op_file_dirly != NULL) {
		if (g_core_fp.fp_0f_op_file_dirly(g_fw_boot_upgrade_name))
			E("%s: update FW fail\n", __func__);
	}
#else
...
static uint8_t himax_mcu_tp_info_check(void)
{
	...
	if (ic_data->HX_STYLUS_FUNC) {
		...
		if (ic_data->HX_STYLUS_ID_V2 != data[2]) {
			err_cnt++;
			W("%s: STYLUS_ID_V2, Set = %d ; FW = %d", __func__,
				ic_data->HX_STYLUS_ID_V2, data[2]);
		}
		if (ic_data->HX_STYLUS_RATIO != data[3]) {
			err_cnt++;
			W("%s: STYLUS_RATIO, Set = %d ; FW = %d", __func__,
				ic_data->HX_STYLUS_RATIO, data[3]);
		}
	}
...
static void himax_mcu_touch_information(void)
{
 	if (t_lcm_name == 4){
                if (ic_data->HX_RX_NUM == 0xFFFFFFFF)
                        ic_data->HX_RX_NUM = FIX_HX_RX_NUM_BOE;

                if (ic_data->HX_TX_NUM == 0xFFFFFFFF)
                        ic_data->HX_TX_NUM = FIX_HX_TX_NUM_BOE;
        }else if (t_lcm_name == 3){
                if (ic_data->HX_RX_NUM == 0xFFFFFFFF)
...
void himax_mcu_clean_sram_0f(const uint8_t *addr, int write_len, int type)
{
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if ( type ==0 )
	{
		...
		if( private_ts->g_fw_sta == true)
		{
			...
			I("%s 01, Entering,file name = %s\n", __func__, file_name);
			reqret = request_firmware(&fw, file_name, private_ts->dev);
			if (reqret < 0) {
#if defined(HX_FIRMWARE_HEADER)
				fw = &g_embedded_fw;
				I("%s 01: Not find FW in userspace, use embedded FW(size:%zu)\n",
					__func__, g_embedded_fw.size);
#else
				ret = reqret;
				E("%s 01: request firmware fail, code[%d]!!\n", __func__, ret);
				goto END;
#endif
			}
		}
#else
	I("%s 02, Entering,file name = %s\n", __func__, file_name);
	reqret = request_firmware(&fw, file_name, private_ts->dev);
	if (reqret < 0) {
#if defined(HX_FIRMWARE_HEADER)
				fw = &g_embedded_fw;
				I("%s 02: Not find FW in userspace, use embedded FW(size:%zu)\n",
					__func__, g_embedded_fw.size);
#else
				ret = reqret;
				E("%s 02: request firmware fail, code[%d]!!\n", __func__, ret);
				goto END;
#endif
	}
#endif
	} else {
		I("%s 03, Entering,file name = %s\n", __func__, file_name);
		reqret = request_firmware(&fw, file_name, private_ts->dev);
		if (reqret < 0) {
			ret = reqret;
			E("%s 03: request firmware fail, code[%d]!!\n", __func__, ret);
			goto END;
		}
	}
