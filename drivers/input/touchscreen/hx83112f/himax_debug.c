...
static void _str_to_arr_in_char(char **arr, int arr_size,
	...
	if (strnstr(buf, "help", len) != NULL) {
		...
		for (i = 0; i < 20; i++) {
			if (buf[i] == ',' || buf[i] == '\n') {
				...
				if (k < 4) {
					if (kstrtoul(buf_tmp, 10, &value) == 0)
						layout[k++] = value;
					else
						I("kstrtoul convert fail\n");
				}
			}
		}
...
{
	diag_mutual_2 =
	  kzalloc(x_num * y_num * sizeof(int32_t), GFP_KERNEL);
}
#endif
...
void himax_log_touch_data(struct himax_ts_data *ts, int start)
{
	...
	if (debug_data->is_stack_full_raw)
		print_size = HX_FULL_STACK_RAWDATA_SIZE;
	else
		print_size = 128;
