	...
	const struct firmware *file_entry, int tx_num, int rx_num)
{
	...
	if (g_inspection_criteria[crtra_id] != NULL) {
	...
	g_inspection_criteria[crtra_id] = kcalloc((tx_num
				* rx_num),
				sizeof(int), GFP_KERNEL);
	if (g_inspection_criteria[crtra_id] == NULL) {
...
/* 	I("%s:g_inspt_crtra[%d]=%s, len=%5lu\n", __func__,
		crtra_id,
		g_hx_inspt_crtra_name[crtra_id],
		(unsigned long)(tx_num * rx_num) * sizeof(int));	 */
...
static int himax_self_test_data_init(void)
{
	...
	if (t_lcm_name == 3)
		file_name_2 = "dsbj_hx_criteria.csv";
	else if (t_lcm_name == 4)
		file_name_2 = "djn_hx_criteria.csv";
	I(" %s: t_lcm_name = %d\n",__func__,t_lcm_name);
	/*
...
//	I("g_rslt_data , len=%5lu\n", (unsigned long)g_1kind_raw_size * count_item * sizeof(char));
