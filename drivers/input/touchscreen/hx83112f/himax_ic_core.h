...
int himax_report_data_2(struct himax_ts_data *ts, int ts_path, int ts_status);
...
/* CORE_DRIVER */
	...
	void (*fp_usb_detect_set)(const uint8_t *cable_config);
...
/* CORE_DRIVER */
...
#if defined(HX_ZERO_FLASH)
	void (*fp_clean_sram_0f)(const uint8_t *addr, int write_len, int type);
	void (*fp_write_sram_0f)(const uint8_t *addr, const uint8_t *data,
		uint32_t len);
	...
	void (*fp_read_sram_0f)(const struct firmware *fw_entry,
			...
			int start_index,
			int read_len);
	void (*fp_read_all_sram)(const uint8_t *addr, int read_len);
