        ...
        u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
 ...
 //       if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_HKC)) {
#endif
            firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
            cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME_HKC,
#else
            CFG_CTS_FIRMWARE_FILENAME_HKC,
#endif
            firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
 //       }
    } else if (cts_lcm_name == 9) {
#ifndef CFG_CTS_FOR_GKI
 //       if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH_MDT)) {
#endif
            firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
            cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME_MDT,
#else
            CFG_CTS_FIRMWARE_FILENAME_MDT,
#endif
            firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
//       }
    }
