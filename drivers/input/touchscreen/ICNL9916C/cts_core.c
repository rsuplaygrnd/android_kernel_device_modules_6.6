...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    if (cts_lcm_name == 8) {
        sprintf(Ctp_name,"XINXIAN,ICNL9916,FW:0x%04x",fwdata->version);
    }else {
        sprintf(Ctp_name,"DSBJ,ICNL9916,FW:0x%04x",fwdata->version);
    }
    cts_info("cts_tcs_get_fw_ver Ctp_name is : %s",Ctp_name);
