...
#endif
	platdata->irq_gpio = of_get_named_gpio(np, "novatek,irq-gpio", 0);
...
//	if (ts->platdata->support_ear_detect)
	...
	if (ret) {
		input_err(true, &ts->client->dev, "register mtk_disp_notifier failed\n");
		goto err_register_suspend_resume_failed;
	}
#endif
...
static void nvt_ts_remove(struct spi_device *client)
{
	...
	mtk_disp_notifier_unregister(&ts->disp_notifier);
#endif
	usb_unregister_notifier_chain_for_tp(&ts->notifier_charger);
#if SEC_TOUCH_CMD
	nvt_ts_sec_fn_remove(ts);
#endif
#if NVT_TOUCH_MP
...
static int nvt_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	int plug_flag = 0;
	plug_flag = (int)val;
	input_info(true, &ts->client->dev, "%s event id =%d\n", __func__, plug_flag);
	if(plug_flag == 0 || plug_flag == 1){
		if (ts->usb_plug_status != plug_flag){
			ts->usb_plug_status = plug_flag;
...
void nvt_plat_charger_init(void)
{
	...
	ret = usb_register_notifier_chain_for_tp(&ts->notifier_charger);
	if (ret < 0)
...
static int32_t __init nvt_driver_init(void)
{
	...
	pr_info("[sec_input] wt_saved_command_line is %s \n", wt_saved_command_line);
	if (strstr(wt_saved_command_line,"n28_nt36528_dsi_vdo_hdp_truly_truly")){
		nvt_lcm_name = 1;
	}else{
		pr_info("[sec_input] lcm match is failed\n");
		return  -1;
	}
