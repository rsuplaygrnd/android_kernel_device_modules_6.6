...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	...
	{.desp = "raw adaptability x", .catalog = RAW_ADAPTABILITY, .lcm = ON},
	{.desp = "raw adaptability y", .catalog = RAW_ADAPTABILITY, .lcm = ON},
	/* LCM OFF TEST */
...
/* char M_CFG_NIS = ':'; */
	...
	memset(seq_item, 0, sizeof(seq_item) * sizeof(char));
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (tItems[index].catalog == RAW_ADAPTABILITY) {
		if (core_mp.nPenChipnum == 0xFF || core_mp.nPenChipnum == 0) {
			ILI_ERR("The pen_chip_num is invalid , value = (%x)\n", core_mp.nPenChipnum);
			ret = -EMP_INVAL;
			goto out;
		}
	}
...
int ili_parsing_sram_param(void)
{
	...
	ILI_INFO("Free Run TPU Clock EN : %s, Sram Group EN : %s\n", ilits->sram_para.tpu_clock_en ? "ENABLE" : "DISABLE", ilits->sram_para.sram_group_en ? "ENABLE" : "DISABLE");
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].catalog == RAW_ADAPTABILITY) {
			if(ipio_strcmp(tItems[i].desp, "raw adaptability y") == 0) {
				core_mp.xch_len = core_mp.nPyRaw * core_mp.nPenChipnum;
			} else {
				core_mp.ych_len = core_mp.nPxRaw * core_mp.nPenChipnum;
			}
			core_mp.frame_len = core_mp.xch_len * core_mp.ych_len;
		}
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	if (ilits->PenType == POSITION_PEN_TYPE_ON) {
		...
		core_mp.nPenChipnum = ilits->pen_info_block.nPenChipnum;
		core_mp.nPenSamplenum = ilits->pen_info_block.nPenSamplenum;
	}
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		if (tItems[i].catalog == MUTUAL_TEST) {
			...
			tItems[i].do_test = peak_to_peak_test;
		} else if (tItems[i].catalog == RAW_ADAPTABILITY) {
			tItems[i].do_test = mutual_test;
		}
...
static void mp_test_run(int index)
{
	...
	if (tItems[i].catalog == RAW_ADAPTABILITY) {
		if (ipio_strcmp(tItems[index].desp, "raw adaptability y") == 0) {
			core_mp.xch_len = core_mp.nPyRaw * core_mp.nPenChipnum;
		} else {
			core_mp.ych_len = core_mp.nPxRaw * core_mp.nPenChipnum;
		}
