...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	case DATA_FORMAT_DEBUG:
		...
		if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_AXIS) {
			len += P5_X_CUSTOMER_AXIS_LENGTH;
		} else if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_ALS) {
			len += P5_X_CUSTOMER_ALS_LENGTH;
		}
	...
	case DATA_FORMAT_DEBUG_LITE_PEN:
		...
		ctrl = DATA_FORMAT_DEBUG_LITE_CMD;
		break;
	case DATA_FORMAT_DEBUG_LITE_PEN_AREA:
		debug_ctrl = DATA_FORMAT_DEBUG_LITE_PEN_AREA_CMD;
		ctrl = DATA_FORMAT_DEBUG_LITE_CMD;
		cmd[4] = data[1];
		cmd[5] = data[2];
		cmd[6] = data[3];
	...
	if (ctrl == DATA_FORMAT_DEBUG_LITE_CMD) {
		if (format == DATA_FORMAT_DEBUG_LITE_PEN || format == DATA_FORMAT_DEBUG_LITE_PEN_AREA)
		...
		if (ilits->chip->core_ver >= CORE_VER_1700) {
			...
			cmd[2] = data_type;
			cmd[3] = debug_ctrl;
			ret = ilits->wrapper(cmd, 12, NULL, 0, ON, OFF);
		} else {
