...
bool gestrue_spay = DISABLE;
bool sensitivity_status = DISABLE;
...
{
	struct touch_bus_info *info;
	ILI_INFO("ili_interface_dev_init\n");

	if (strstr(wt_saved_command_line,"ili7807s_fhdp_wt_dsi_vdo_cphy_90hz_tianma")) {
		g_lcm_name = 2;//ili7807s 2
	}
	else if (strstr(wt_saved_command_line,"ili7807s_fhdp_wt_dsi_vdo_cphy_90hz_chuangwei")) {
		g_lcm_name = 5;
	}
	else
	{
		ILI_INFO("TP match ili7807s fail\n");
		return -1;
	}
