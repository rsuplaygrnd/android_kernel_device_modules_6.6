...
static int ili_check_polling_bist(u8 GroupNumIdx, u32 SramRsultStatus, u32 mask)
{
	u32 Bist_Sram_Fail = 0xFFFFFFFF;
	int CheckCount = 10, ret = 0;
	u32 bist_done = 0, bist_fail = 0;
	...
	if(ilits->sram_para.sram_group_en == ENABLE) {
		...
		if ((Bist_Sram_Fail & SramRsultStatus & mask) > 0) {
			ILI_ERR("Check Bist Fail Flag Fail!\n");
			ILI_ERR("Bist Sram Fail Status = 0x%X, Sram Rsult Status = 0x%X, mask = 0x%X\n", Bist_Sram_Fail, SramRsultStatus, mask);
			ILI_ERR("Check Bist_Fail_Flag Fail!!,FailValue= = 0x%X\n", Bist_Sram_Fail & SramRsultStatus & mask);
			ret = -1;
			goto out;
		}
	} else {
...
static int ili_sram_by_partial_area_test(void)
{
	...
	for(nSramNumIdx = 0; nSramNumIdx < ilits->sram_para.group_num; nSramNumIdx++) {
		...
		if (ili_check_polling_bist(nSramNumIdx, ilits->sram_para.SramResult[nSramNumIdx], 0xFFFFFFFF) < 0) {
...
static int ili_ic_cascade_polling_bist(void)
{
	int retry = 10, ret = 0;
	u32 bist_done = 0, bist_fail = 0;
	u32 bist_slave_done = 0, bist_slave_fail = 0;
...
static int ili_IL79451A_sram_test(void)
{
	int ret = 0, i;
	u8 nSramNumIdx;
	u32 mask[2][2] = {{0x05, 0x60}, {0x06, 0x18}};
	u16 max_addr[2][2] = {{0x27FF, 0x27}, {0x27FF, 0x7F}};

	max_addr[0][0] = ilits->sram_para.MaxAddr;
	max_addr[1][0] = ilits->sram_para.MaxAddr;
	for (nSramNumIdx = 0; nSramNumIdx < ilits->sram_para.group_num; nSramNumIdx++) {
		for (i = 0; i < 2; i++) {

			ILI_DBG("Reset \n");
			if (ili_reset_ctrl(ilits->reset) < 0) {
				ILI_ERR("Reset Failed\n");
				ret = -1;
				goto out;
			}
			...
			ili_ice_mode_write_by_mode(0x04605C, 0x00, 1, BOTH); //reg_bist_start_clear_disable

			/* 0x04603C 0x65 or 0x04603C 0x1E*/
			ili_ice_mode_write_by_mode(ilits->sram_para.write_addr, ilits->sram_para.SramPartialAreaTest_en[nSramNumIdx], 4, BOTH);
			ILI_DBG("Write Sram Partial Area En : 0x%X\n", ilits->sram_para.SramPartialAreaTest_en[nSramNumIdx]);


			ili_sram_test(max_addr[nSramNumIdx][i], ilits->sram_para.MinAddr);
			ILI_DBG("max_addr : 0x%X, min_addr : 0x%X\n", max_addr[nSramNumIdx][i], ilits->sram_para.MinAddr);

			if (ili_check_polling_bist(nSramNumIdx, ilits->sram_para.SramResult[nSramNumIdx], mask[nSramNumIdx][i]) < 0) {
				ILI_ERR("Check March_LR Bist Fail during partial area test\n");
				ret = -1;
				goto out;
			}

			ili_ice_mode_write_by_mode(0x04605C, 0x01, 1, BOTH);
		}
	}
out:
	return ret;
...
int ili_tddi_ic_sram_test(void)
{
	...
	if (ilits->chip->id == ILI79451) {
		ret = ili_IL79451A_sram_test();
		goto out;
	}
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		ret = ili_check_polling_bist(false, false, 0xFFFFFFFF);
	}
out:
...
int ili_ic_get_project_id(u8 *pdata, int size)
{
	...
	u8 cmd;
	u32 customerFlashAddrStart = 0x0;

	if (ilits->fw_upgrade_mode == UPGRADE_FLASH) {
		if (!pdata) {
			ILI_ERR("pdata is null\n");
			return -ENOMEM;
		}
		/* To get block info must be after FW running */
		if (ilits->chip->core_ver >= CORE_VER_2100) {
			do {
				cmd = P5_X_GET_BLOCK_INFOMATION;
		...
		if (block_info[0] == P5_X_GET_BLOCK_INFOMATION) {
			ili_dump_data(block_info, 8, 20, 10, "CUSTOMER");
			customerFlashAddrStart = (block_info[3] << 16) + (block_info[2] << 8) + block_info[1];
		} else {
			customerFlashAddrStart = RSV_BK_ST_ADDR;
		}
		ILI_INFO("BK_ST_addr = %x\n", customerFlashAddrStart);

		ice = atomic_read(&ilits->ice_stat);
		...
		if (ili_ice_mode_write(0x041008, (customerFlashAddrStart & 0xFF0000) >> 16, 1) < 0)
			ILI_ERR("Write address failed\n");
		if (ili_ice_mode_write(0x041008, (customerFlashAddrStart & 0x00FF00) >> 8, 1) < 0)
			ILI_ERR("Write address failed\n");
		if (ili_ice_mode_write(0x041008, (customerFlashAddrStart & 0x0000FF), 1) < 0)
			ILI_ERR("Write address failed\n");
...
out:
	if(g_lcm_name ==2) {
	sprintf(Ctp_name,"TM,ILI7807S,FW:0x0%x\n",buf[3]);
	}
	else if(g_lcm_name ==5) {
	sprintf(Ctp_name,"CW,ILI7807S,FW:0x0%x\n",buf[3]);
	} else {
	sprintf(Ctp_name,"TM,ILI7807S,FW:0x0%x\n",buf[3]);
	}
...
int ili_ic_get_panel_info(void)
{
	...
	do {
		...
		ILI_INFO("cmd = 0x%X, buf[0] = 0x%X, buf[1] = 0x%X, buf[2] = 0x%X, buf[3] = 0x%X, buf[4] = 0x%X, buf[5] = 0x%X, buf[6] = 0x%X, buf[7] = 0x%X\n", cmd, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);
		if (ret < 0)
		...
		if (buf[0] != cmd) {
			...
			ilits->pen_info_block.nPenChipnum = 6;
			ilits->pen_info_block.nPenSamplenum = 22;
		} else {
			...
			ilits->pen_info_block.nPenChipnum = buf[6];
			ilits->pen_info_block.nPenSamplenum = buf[7];
			break;
		}
	} while (++retry < 3);
	retry = 0;
...
out:
	...
	if (ilits->chip->core_ver >= CORE_VER_1700) {
		ILI_INFO("Pen Type = %X\n", ilits->PenType);
		ILI_INFO("Pen Info Data, nPxRaw = %d, nPyRaw = %d, nPxVa = %d, nPyVa = %d, nPenX_MP = %d, nPenChipnum = %d, nPenSamplenum = %d\n", ilits->pen_info_block.nPxRaw, ilits->pen_info_block.nPyRaw, ilits->pen_info_block.nPxVa, ilits->pen_info_block.nPyVa, ilits->pen_info_block.nPenX_MP, ilits->pen_info_block.nPenChipnum, ilits->pen_info_block.nPenSamplenum);
...
void ili_ic_get_report_info(void)
{
	...
	if ( ilits->chip->core_ver >= CORE_VER_2100 ) {
		...
		do {
			...
			ret = ilits->wrapper(&cmd, sizeof(cmd), buf, 6, ON, OFF);
			if (ret < 0)
...
int ili_ic_get_all_info(void)
{
	...
	if (ilits->wrapper(&cmd, sizeof(cmd), buf, 97, ON, OFF) < 0) {
	...
	if (buf[0] == P5_X_GET_ALL_INFORMATION) {
		...
		if (buf[57] != cmd) {
			...
			ilits->pen_info_block.nPenChipnum = buf[63];
			ilits->pen_info_block.nPenSamplenum = buf[64];
		}
	...
	if (get_all_info == true) {
		...
		ILI_INFO("Pen Len Info : PxRaw = %d, PyRaw = %d, PxVa = %d, PyVa = %d, nPenX_MP = %d, nPenChipnum = %d, nPenSamplenum = %d\n", ilits->pen_info_block.nPxRaw, ilits->pen_info_block.nPyRaw, ilits->pen_info_block.nPxVa, ilits->pen_info_block.nPyVa, ilits->pen_info_block.nPenX_MP, ilits->pen_info_block.nPenChipnum, ilits->pen_info_block.nPenSamplenum);
...
int ili_ic_check_debug_lite_support(int format, bool send, u8 *data)
{
	...
	ret = ilits->wrapper(cmd, 10, buf, 1, ON, OFF);
	if (ret < 0) {
