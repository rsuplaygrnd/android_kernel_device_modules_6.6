...
int ft8722_tpd_enter_tui(void)
{
	int ret = 0;

	pr_err("[%s] enter TUI", __func__);

/* 	atomic_set(&gt9886_tui_flag, true); */

	return ret;
}
EXPORT_SYMBOL(ft8722_tpd_enter_tui);

int ft8722_tpd_exit_tui(void)
{
	int ret = 0;

	pr_err("[%s] exit TUI", __func__);
/* 
...
EXPORT_SYMBOL(ft8722_tpd_exit_tui);
#endif
/*****************************************************************************
...
static int ft8722_charger_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
...
static int fts_charge_notifier_callback_init(struct fts_ts_data *ts_data)
{
	...
	ts_data->notifier_charger.notifier_call = ft8722_charger_notifier_callback;
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	if (r >= 0) {
		...
		register_tpd_tui_request(ft8722_tpd_enter_tui, ft8722_tpd_exit_tui);
	}
#endif
...
int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    cancel_work_sync(&ts_data->resume_work);
    usb_unregister_notifier_chain_for_tp(&ts_data->notifier_charger);
