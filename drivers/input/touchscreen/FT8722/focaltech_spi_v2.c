...
static int __init fts_ts_spi_init(void)
{
    int ret = 0;
    FTS_FUNC_ENTER();
    if (strstr(wt_saved_command_line,"ft8722_fhdp_wt_dsi_vdo_cphy_90hz_txd_sharp")){
        f_tp_name=23;
    }else if (strstr(wt_saved_command_line,"ft8722_fhdp_wt_dsi_vdo_cphy_90hz_txd")){
        f_tp_name=1;
    }else{
        FTS_INFO("TP match ft8722 fail\n");
        return -1;
    }
    FTS_INFO("f_tp_name=%d\n",f_tp_name);
