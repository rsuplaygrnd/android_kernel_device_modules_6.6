...
#define OVT_PROC_TOUCHSCREEN_FOLDER                 "touchscreen"
#define OVT_PROC_CTP_OPENSHORT_TEST_FILE            "ctp_openshort_test"
...
//	struct ovt_tcm_app_info *app_info1;
	struct ovt_tcm_hcd *tcm_hcd = testing_hcd->tcm_hcd;
	unsigned int size;
	unsigned char limit;
	unsigned char *buf;
...
//	app_info1 = &tcm_hcd->app_info;

#ifdef PT1_GET_PIN_ASSIGNMENT
	satic_cfg_length = le2_to_uint(tcm_hcd->app_info->static_config_size);
...
/*	rtc_time_to_tm(get_seconds(), &rtc_now_time);
	...
	if (error_count) {
		//test fail result
		sprintf(file_path, "/data/vendor/fac_sources/tp_%s_test_data_%02d%02d%02d-%02d%02d%02d-fail.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	} else {
		//test pass result
		sprintf(file_path, "/data/vendor/fac_sources/tp_%s_test_data_%02d%02d%02d-%02d%02d%02d-success.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	}
*/  
	if (error_count) {
		//test fail result
		sprintf(file_path, "/data/vendor/fac_sources/tp_%s_test_data-fail.csv", tcm_hcd->id_info.part_number);
	} else {
		//test pass result
		sprintf(file_path, "/data/vendor/fac_sources/tp_%s_test_data-success.csv", tcm_hcd->id_info.part_number);
	}
//  old_fs = get_fs();
//  set_fs(KERNEL_DS);
...
struct proc_dir_entry *ovt_proc_touchscreen_dir;
struct proc_dir_entry *ovt_proc_ctp_openshort_test_file = NULL;
/*proc init part*/
static int  proc_node_init(void)
{
    ...
    if (ovt_proc_touchscreen_dir == NULL)  {
        ...
        ovt_proc_ctp_openshort_test_file = proc_create(OVT_PROC_CTP_OPENSHORT_TEST_FILE,
		(S_IWUSR | S_IRUGO), ovt_proc_touchscreen_dir, &proc_ctp_openshort_test_fops);
        if(ovt_proc_ctp_openshort_test_file ==NULL){
            printk(KERN_ERR "%s: proc_ctp_openshort_test_file create failed!\n", __func__);
            remove_proc_entry(OVT_PROC_TOUCHSCREEN_FOLDER, NULL);
        }else{
 ...
 int  testing_module_init(void)
{
	...
	ret = proc_node_init();
	if (ret < 0){
		printk("%s:proc_node_init fail", __func__);
	}
