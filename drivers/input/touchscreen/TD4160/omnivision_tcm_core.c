...
DECLARE_COMPLETION(ovt_pm_resume_completion);
bool ovt_pm_suspend;
struct wakeup_source *ovt_ws;
...
\
	if (id == 0x09) { \
		n28_gesture_status = buf[0];\
		tcm_hcd->wakeup_gesture_enabled = buf[0];\
		LOGE(tcm_hcd->pdev->dev.parent, "set n28_gesture_status = %d\n",n28_gesture_status); \
	} \
\
...
static void glove_mode(void *device_data)
{
	...
	if(retval < 0){
		LOGE(g_tcm_hcd->pdev->dev.parent,"failed to set glove mode\n");
...
#if WAKEUP_GESTURE
	if (ovt_pm_suspend) {
		retval = wait_for_completion_timeout(&ovt_pm_resume_completion, msecs_to_jiffies(700));
		if (!retval) {
			LOGE(tcm_hcd->pdev->dev.parent,"ovt Bus don't resume from pm(deep),timeout,skip irq\n");
			return IRQ_HANDLED;
		}
	}
	__pm_wakeup_event(ovt_ws, jiffies_to_msecs(5000));
#endif
...
#if MTK_DRM_NOTIFY
static int ovt_tcm_drm_notifier_callback(struct notifier_block *nb,
		unsigned long event, void *data)
{
	...
	if (!blank) {
		LOGE(tcm_hcd->pdev->dev.parent,"Invalid blank\n");
		return -1;
	}

	if (event == MTK_DISP_EARLY_EVENT_BLANK) {
		if (*blank == MTK_DISP_BLANK_POWERDOWN) {
			LOGE(tcm_hcd->pdev->dev.parent,"suspend,event=%lu, *blank=%d\n", event, *blank);
			ovt_tcm_suspend(&tcm_hcd->pdev->dev);
			tcm_hcd->fb_ready = 0;
		}
	} else if (event == MTK_DISP_EVENT_BLANK) {
		if (*blank == MTK_DISP_BLANK_UNBLANK) {
			LOGE(tcm_hcd->pdev->dev.parent,"resume,event=%lu, *blank=%d\n", event, *blank);
//			ovt_tcm_resume(&tcm_hcd->pdev->dev);
			queue_work(tcm_hcd->charger_notify_wq_ovt,&tcm_hcd->ovt_resume_work);
			tcm_hcd->fb_ready++;
//			queue_work(ts->charger_notify_wq,&ts->nvt_resume_work);
		}
	}
...
//	int retval = 0;
	struct ovt_tcm_hcd *tcm_hcd =
	container_of(work, struct ovt_tcm_hcd, update_plug_status_work);

	LOGN(tcm_hcd->pdev->dev.parent,"enter\n");
	if (!tcm_hcd) {
		LOGN(tcm_hcd->pdev->dev.parent,"tcm_hcd is NULL\n");
		return;
	}

	if(tcm_hcd->usb_plug_status == 0 || tcm_hcd->usb_plug_status == 1){
		tcm_hcd->func_charger_connected_en = tcm_hcd->usb_plug_status;
		tcm_hcd->set_dynamic_config(tcm_hcd, DC_CHARGER_CONNECTED, tcm_hcd->func_charger_connected_en);
	} else if (tcm_hcd->usb_plug_status == 2 || tcm_hcd->usb_plug_status == 3) {
		tcm_hcd->func_ear_phone_connected_en = tcm_hcd->usb_plug_status - 2;
		tcm_hcd->set_dynamic_config(tcm_hcd, DC_ENABLE_EAR_PHONE, tcm_hcd->func_ear_phone_connected_en);
	} else {
		LOGN(tcm_hcd->pdev->dev.parent,"tcm usb_online usb_plug_status=%d\n",tcm_hcd->usb_plug_status);
	}

}
static int tcm_usb_notifier_callback(struct notifier_block *nb,
        unsigned long action, void *data)
{
	int plug_flag = 0;
	...
	LOGN(tcm_hcd->pdev->dev.parent,"event id =%d\n", plug_flag);
	if (!tcm_hcd) {
		LOGE(tcm_hcd->pdev->dev.parent,"tcm_hcd is NULL\n");
		return -1;
	}

	if(plug_flag == 0 || plug_flag == 1){
		if (tcm_hcd->usb_plug_status != plug_flag){
			tcm_hcd->usb_plug_status = plug_flag;
			if (tcm_hcd->charher_workqueue != NULL){
				queue_work(tcm_hcd->charher_workqueue, &tcm_hcd->update_plug_status_work);
			}
		}
...
static int tcm_init_usb_notifier(struct ovt_tcm_hcd *tcm_hcd)
{
	int ret = 0;
	tcm_hcd->usb_plug_status = 0;

	LOGN(tcm_hcd->pdev->dev.parent,"enter\n");
	tcm_hcd->usb_notifier.notifier_call = tcm_usb_notifier_callback;

	ret = usb_register_notifier_chain_for_tp(&tcm_hcd->usb_notifier);
	if (ret) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"TCM register usb notifier failed\n"
		);
	}
	return ret;
}

void ovt_resume_workqueue_callback(struct work_struct *work)
{
 	struct ovt_tcm_hcd *tcm_hcd = container_of(work, struct ovt_tcm_hcd, ovt_resume_work);
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_resume_workqueue_callback start\n");
	ovt_tcm_resume(&tcm_hcd->pdev->dev);
...
/*create /sys/touchscreen */
	...
	retval = sec_cmd_init(&tcm_hcd->sec, sec_cmds, ARRAY_SIZE(sec_cmds), SEC_CLASS_DEVT_TSP);
	if (retval < 0){
		LOGE(tcm_hcd->pdev->dev.parent,"failed to sec cmd init\n");
		return retval;
	}
...
*/
...
#if MTK_DRM_NOTIFY
	tcm_hcd->disp_notifier.notifier_call = ovt_tcm_drm_notifier_callback;
	retval = mtk_disp_notifier_register("ovt_ts_notifier", &tcm_hcd->disp_notifier);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to register FB notifier client\n");
	}
#endif
	retval = tcm_init_usb_notifier(tcm_hcd);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	tcm_hcd->charger_notify_wq_ovt = create_singlethread_workqueue("ovt_charger_wq");
	INIT_WORK(&tcm_hcd->ovt_resume_work, ovt_resume_workqueue_callback);
	...
	tcm_hcd->charher_workqueue =
			create_singlethread_workqueue("ovt_tcm_charger_detect_workqueue");
	LOGE(tcm_hcd->pdev->dev.parent,
			"create charger detect workqueue\n");
	INIT_WORK(&tcm_hcd->update_plug_status_work, update_plug_status);
	...
	ovt_ws = wakeup_source_register(&pdev->dev,"ovt_wakelock");
	if (!ovt_ws)
		LOGE(tcm_hcd->pdev->dev.parent,"ovt_wakelock source request failed\n");
	ovt_pm_suspend = false;
//    init_completion(&ovt_pm_resume_completion);
...
//	struct drm_panel *active_panel = tcm_get_panel();
	...
	if (tcm_hcd->charger_notify_wq_ovt) {
		flush_workqueue(tcm_hcd->charger_notify_wq_ovt);
		destroy_workqueue(tcm_hcd->charger_notify_wq_ovt);
	}
	...
	if (ovt_ws)
		wakeup_source_unregister(ovt_ws);
...
#endif
*/
	usb_unregister_notifier_chain_for_tp(&tcm_hcd->usb_notifier);
#if MTK_DRM_NOTIFY
...
#ifdef CONFIG_PM
static int ovt_suspend(struct device *dev)
{
	ovt_pm_suspend= true;
	reinit_completion(&ovt_pm_resume_completion);
	return 0;
}

static int ovt_resume(struct device *dev)
{
	ovt_pm_suspend = false;
	complete(&ovt_pm_resume_completion);
	return 0;
}
#endif
...
static int __init ovt_tcm_module_init(void)
{
	...
	if(strstr(wt_saved_command_line,"n28_td4160_dsi_vdo_hdp_xinxian_inx")){
		ovt_lcm_name = 3;
	}else if(strstr(wt_saved_command_line,"n28_td4160_dsi_vdo_hdp_boe_boe")){
		ovt_lcm_name = 7;
	}else{
		printk("ovt_tcm_module_init lcm match is failed");
		return -1;
	}
