...
struct TEEC_UUID fpc_uuid = { 0x04010000, 0x0000, 0x0000, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} };
#endif
...
static void fpc_free(void)
{
    ...
    while(gpio_count) {
        if(gpio_is_valid(fpc_gpio[gpio_count - 1])) {
			...
			gpio_free(fpc_gpio[gpio_count - 1]);
...
static void fpc_id_read(void)
{
    ...
    pr_info("%s, id = %2x %2x %2x, status = %d\n", __func__, buf[0], buf[1], buf[2], status);
...
static void fpc_gpio_set(int vdd, int rst, int irq)
{
    ...
    pr_info("%s, fpc_irq_num = %d, value = %d, status = %d\n", __func__, fpc_irq_num, gpio_get_value(irq), status);
}
...
static void fpc_power_regulator(void)
{
    ...
    if (of_property_read_bool(fpc_node, "vfp-supply")) {
        ...
        if(status) {
            pr_err("err %s +%d, 7.15 status = %d\n", __func__, __LINE__, status);
            return;           
        }
        status = regulator_enable(fpc_regulator);
        if(status) {
            pr_err("err %s + regulator_enable, status = %d\n", __func__, status);
            return;           
        }
        pr_info("%s, fpc_vdd = %d\n", __func__, fpc_vdd);
    } else {
...
static void fpc_spi_remove(struct spi_device *dev)
{
    (void)dev;
}
...
/*
        case FPC_IOC_READ_SYS:
            ...
            status = copy_to_user((char __user *)(uintptr_t)arg, fpc_sys_path, strlen(fpc_sys_path));
            CHECK_STATUS(status)
            kfree(fpc_sys_path);
            break;
*/
...
static ssize_t fpc_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if (strncmp(cmd, "id", 2) == 0) {
        ...
        status = kstrtouint(cmd, 10, &fpc_vdd);
		CHECK_STATUS(status)
        fpc_power_regulator();
        fpc_pinctrl();
        fpc_gpio_request();
...
static int fpc_dev_create(void)
{
    ...
    CHECK_STATUS(status)
    fpc_class = class_create(FPC_MODULE_NAME);
    if (IS_ERR(fpc_class)) {
