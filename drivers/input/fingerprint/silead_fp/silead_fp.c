                             ...
                             unsigned long event, void *data)
{
    ...
    LOG_MSG_DEBUG(DBG_LOG, "[%s] enter 1031 \n", __func__);
    ...
    case MTK_DISP_BLANK_UNBLANK:
        LOG_MSG_DEBUG(DBG_LOG, "[%s] LCD ON\n", __func__);
        fp_dev->scr_off = 0;
        silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_ON);
        break;

    case MTK_DISP_BLANK_POWERDOWN:
        LOG_MSG_DEBUG(DBG_LOG, "[%s] LCD OFF\n", __func__);
        fp_dev->scr_off = 1;
        silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_OFF);
        break;

    default:
        LOG_MSG_DEBUG(DBG_LOG, "[%s] Unknown notifier\n", __func__);
        break;
    }
    return retval;
}
...
static irqreturn_t silfp_irq_handler(int irq, void *dev_id)
{
    ...
    if (true) {
        ...
        wake_lock_timeout(&fp_dev->wakelock, 3000); /* set a little long for a poor MCU */
...
static int silfp_class_init(void)
{
    ...
    fp_cls = class_create(CLASS_NODE);
...
static int silfp_init(struct silfp_data *fp_dev)
{
	...
	mtk_disp_notifier_register("silead_fp", &fp_dev->notif);
    LOG_MSG_DEBUG(DBG_LOG, "[%s] mtk_disp_notifier_register\n", __func__);
#endif /* CONFIG_HAS_EARLYSUSPEND */

    atomic_set(&fp_dev->spionoff_count,0);
	
    LOG_MSG_DEBUG(DBG_LOG, "[%s] exit\n", __func__);
    return status;
}
...
static void silfp_exit(struct silfp_data *fp_dev)
{
	...
	mtk_disp_notifier_unregister(&fp_dev->notif);
#endif /* CONFIG_HAS_EARLYSUSPEND */
...
{
    if (lock) {
        wake_lock_timeout(&fp_dev->wakelock_hal, 3000);
    } else {
...
int  silfp_touch_event_handler(struct fp_dev_touch_info* tp_info)
{
    ...
    if (g_fp_dev) {
        wake_lock_timeout(&g_fp_dev->wakelock, 3000);
    }
    return 0;
}
...
static void silfp_remove(struct spi_device *spi)
{
...
int silfp_dev_init(void)
{
    ...
    silfp_class = class_create("silead_fp");
...
MODULE_DESCRIPTION("Silead Fingerprint driver for GSL6xxx/GSL7xxx/GSL8xxx series.");
MODULE_LICENSE("GPL");
