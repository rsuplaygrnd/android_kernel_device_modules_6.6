...
static int fpsensor_fb_notifier_callback(struct notifier_block* self, unsigned long event, void* data)
{
    ...
    blank = *(int*)data;  //*(int*)evdata->data;
...
static struct notifier_block fpsensor_fb_notifier = {
	.notifier_call = fpsensor_fb_notifier_callback,
};
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_REMOVE:
		...
		mtk_disp_notifier_unregister(&fpsensor_fb_notifier);
#endif
...
static int fpsensor_dev_setup(fpsensor_data_t *fpsensor)
{
 ...
 //   }
    ...
    fpsensor->class = class_create(FPSENSOR_CLASS_NAME);
...
*/
	...
	//fpsensor_dev->notifier = &fpsensor_fb_notifier;
	
    //fb_register_client(&fpsensor_dev->notifier);
	mtk_disp_notifier_register("fpsensor_fb",&fpsensor_fb_notifier);
#endif

//+Bug 773955,gwq.wt,add,2022/08/05, add fp adm node
	fingerprint_adm_class = class_create(FINGERPRINT_ADM_CLASS_NAME);
	...
	if(device_create_file(fingerprint_adm_dev,&adm_on_off_attr) < 0){
		fpsensor_debug(ERR_LOG, " %s : fail to creat fingerprint_adm_device  %s \n", __func__,adm_on_off_attr.attr.name);
	}
//-Bug 773955,gwq.wt,add,2022/08/05, add fp adm node
...
#if defined(USE_SPI_BUS)
static void fpsensor_remove(struct spi_device *spi)
#elif defined(USE_PLATFORM_BUS)
static void fpsensor_remove(struct platform_device *spi)
#endif
{
    fpsensor_data_t *fpsensor_dev = g_fpsensor;
    ...
    if (fpsensor_dev->irq)
        free_irq(fpsensor_dev->irq, fpsensor_dev);
#if FP_NOTIFY
    fb_unregister_client(&fpsensor_fb_notifier);
#endif
    fpsensor_dev_cleanup(fpsensor_dev);
