...
#define NUM_PHASES 4
u8 ch_enable_status = 0;
u8 pause_mode_flag[NUM_PHASES] = {0};

#if SMTC_TEST_CHS_EN //通道测试时用全开全关策略
static int sx932x_enable_channel(psx93XX_t this, u8 ch_id, u8 enable)
{
    int ret = 0;
    int ch = 0;
    ...
    {
        if (ch_enable_status){
            pr_info("sx932x:%s() ch_%d and all channels have been enabled\n", __func__, ch_id);
            return 0;
        }
        else
        {
            for(ch = 0; ch < NUM_PHASES; ch++) {
                pause_mode_flag[ch] = 1;
            }

            ch_enable_status = 0xF;
            write_register(this, SX932x_CTRL1_REG, 0x2F);
            pr_info("sx932x:%s() enable all channels\n", __func__);
        }
    }
    else{
        ...
        if (ch_enable_status){
            pr_info("sx932x:%s() disable ch_%d\n", __func__, ch_id);
        }
        else{
            if(pause_mode_flag[ch_id]){
                pr_info("sx932x:%s() ch%d enter the pause mode , will not cali or close.\n",
                    __func__, ch_id);
            }
            else{//it should never be here
                write_register(this, SX932x_CTRL1_REG, 0x20);
                pr_info("sx932x:%s() ch_%d disabled, all channels disabled\n",
                    __func__, ch_id);
            }
        }
...
static int sx932x_enable_channel(psx93XX_t this, u8 ch_id, u8 enable)
{
    ...
    if (enable)
    {
        if (ch_enable_status & 1<<ch_id){
            pr_info("sx932x:%s() ch_%d has been enabled\n", __func__, ch_id);
        }
        else
        {
            pause_mode_flag[ch_id] = 1;
            ch_enable_status |= 1<<ch_id;
            reg_val |= ch_enable_status;

            write_register(this, SX932x_CTRL1_REG, reg_val);
            pr_info("sx932x:%s() enable ch_%d\n", __func__, ch_id);
        }
    }
    else{
        if(pause_mode_flag[ch_id]){
    ...
    for (i = 0; i < ARRAY_SIZE(psmtcButtons); i++) {
        if (!strcmp(temp_input_dev->name, psmtcButtons[i].name)) {
            if (enable) {
                ...
                if (0 == i) {
                    sx932x_enable_channel(this, 0, 1);
                    sx932x_enable_channel(this, 1, 1); //ref phase
                } else {
                    sx932x_enable_channel(this, i, 1);
                }
                read_register(this,SX932x_CTRL1_REG, &reg_val);
                pr_info("sx932x:%s() enable ch_%zu, phen_reg_val= 0X%X\n",
                    __func__, i, reg_val);
            } else {
                psmtcButtons[i].enabled = false;

                if (0 == i) {
                    sx932x_enable_channel(this, 0, 0);
                    sx932x_enable_channel(this, 1, 0); //ref phase
                } else {
                    sx932x_enable_channel(this, i, 0);
                }

                read_register(this,SX932x_CTRL1_REG, &reg_val);
                pr_info("sx932x:%s() disable ch_%zu, phen_reg_val= 0X%X\n",
                    __func__, i, reg_val);
            }
            touchProcess(this);
        }
...
#if 1
static ssize_t sx9328_channel_en_store(const struct class *class, const struct class_attribute *attr, const char *buf, size_t count)
{
    int ch_id = 0;
    int en = 0;
    ...
    if((ch_id >= ARRAY_SIZE(psmtcButtons)) || (ch_id < 0)) {
        pr_info("channel number out of range, the effective number is 0~%lu\n", ARRAY_SIZE(psmtcButtons) - 1);
        return -EINVAL;
    }
...
static ssize_t sx9328_channel_en_show(const struct class *class, const struct class_attribute *attr, char *buf)
{
    int ii = 0;
    char *p = buf;
    int enabled = 0;
...
static int initialize(psx93XX_t this)
{
	...
	if (this) {
		...
		if(0 != ret)
		{
			pr_err("sx932x_global_variable_init failed\n");
		}
...
/* +S96818AA1-1936, liuling3.wt,ADD, 2023/05/29, wt Adaptive sensor hal */
    ...
    if (err < 0) {
        printk("sx932x:register capsense class failed\n");
        return err;
    }
/* -S96818AA1-1936, liuling3.wt,ADD, 2023/05/29, wt Adaptive sensor hal */
...
static void sx932x_remove(struct i2c_client *client)
{
	psx932x_platform_data_t pplatData =0;
	psx932x_t pDevice = 0;
