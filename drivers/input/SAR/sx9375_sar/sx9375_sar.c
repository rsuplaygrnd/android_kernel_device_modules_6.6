    ...
    for (i = 0; i < ARRAY_SIZE(psmtcButtons); i++) {
        if (!strcmp(temp_input_dev->name, psmtcButtons[i].name)) {
            if (enable) {
                ...
                sx937x_i2c_write_16bit(this, SX937X_GENERAL_SETUP, val);
                manual_offset_calibration(this);
                input_report_rel(psmtcButtons[i].sx9375_input_dev, REL_MISC, SAR_STATE_FAR);
                ...
                sx937x_i2c_write_16bit(this, SX937X_GENERAL_SETUP, val);
                manual_offset_calibration(this);
                input_report_rel(psmtcButtons[i].sx9375_input_dev, REL_MISC, SAR_STATE_NONE);
                input_sync(psmtcButtons[i].sx9375_input_dev);
...
static ssize_t ch2_cap_diff_dump_show(const struct class *dev,
                                      const struct class_attribute *attr,
                                      char *buf)
{
    psx93XX_t this = pSx9375Data;
...
static ssize_t ch3_cap_diff_dump_show(const struct class *dev,
                                      const struct class_attribute *attr,
                                      char *buf)
{
    psx93XX_t this = pSx9375Data;
...
static int sx937x_probe(struct i2c_client *client)
{
    ...
    if (this)
    {
        ...
        * (1->NIRQ=0, 0->NIRQ=1) */
        ...
        if (pDevice)
        {
            ...
            for (int i = 0; i < pButtonInformationData->buttonSize; i++) {
				if ((pplatData->button_used_flag >> i) & 0x01) {
					...
					input_report_rel(pButtonInformationData->buttons[i].sx9375_input_dev, REL_MISC, SAR_STATE_NONE);
                    ...
                    pButtonInformationData->buttons[i].sx9375_sensors_class_cdev.name = "SX9375";
    ...
    if (err < 0) {
        printk("sx937x:register capsense class failed\n");
        return err;
    }
