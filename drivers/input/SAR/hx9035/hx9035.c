...
#else
    hx9035_pdata.irq_gpio = of_get_named_gpio(dt_node, "tyhx,irq-gpio", 0);
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9035_channels); ii++) {
        if (strcmp(sensors_cdev->sensor_name, hx9035_channels[ii].name) == 0) {
            if (1 == enable) {
                ...
                input_report_rel(hx9035_channels[ii].hx9035_input_dev, REL_MISC, -1);
                input_report_rel(hx9035_channels[ii].hx9035_input_dev, REL_X, 2);
                input_sync(hx9035_channels[ii].hx9035_input_dev);
            } else {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(hx9035_channels); i++) {
        if (!strcmp(temp_input_dev->name, hx9035_channels[i].name)) {
            ...
            if (val) {
                ...
                input_report_rel(hx9035_channels[i].hx9035_input_dev, REL_MISC, -1);
                input_report_rel(hx9035_channels[i].hx9035_input_dev, REL_X, 2);
                input_sync(hx9035_channels[i].hx9035_input_dev);
                hx9035_channels[i].state = IDLE;
            }
...
static ssize_t hx9035_raw_data_show(struct device *dev,
...
static ssize_t hx9035_register_write_store(struct device *dev,
...
static ssize_t hx9035_register_read_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    unsigned int reg_address = 0;
...
static ssize_t hx9035_channel_en_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ch_id = 0;
    int en = 0;
...
static ssize_t hx9035_channel_en_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int ii = 0;
    char *p = buf;
...
static ssize_t hx9035_manual_offset_calibration_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    hx9035_read_offset_dac();
...
static ssize_t hx9035_manual_offset_calibration_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ii = 0;
    unsigned long val;
...
static ssize_t hx9035_prox_state_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static ssize_t hx9035_polling_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int value = 0;
    int ret = -1;
...
static ssize_t hx9035_polling_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static ssize_t hx9035_loglevel_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static ssize_t hx9035_loglevel_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int value = 0;
...
static ssize_t hx9035_accuracy_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static ssize_t hx9035_accuracy_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int value = 0;
...
static ssize_t hx9035_output_switch_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static ssize_t hx9035_output_switch_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int value = 0;
...
static ssize_t hx9035_threshold1_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ii = 0;
    bool threshold_flag = false;
...
static ssize_t hx9035_threshold1_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int ii = 0;
    char *p = buf;
...
static ssize_t hx9035_thres_alg_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned int en = 0;
...
static ssize_t hx9035_thres_alg_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    char *p = buf;
...
static ssize_t hx9035_dump_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int ret = -1;
    int ii = 0;
...
static ssize_t hx9035_offset_dac_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int ii = 0;
    char *p = buf;
...
static ssize_t hx9035_offset_csn_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int ii = 0;
    char *p = buf;
...
static ssize_t hx9035_reinitialize_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int value = 0;
...
static ssize_t hx9035_hw_monitor_en_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static ssize_t hx9035_hw_monitor_en_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int value = 0;
...
static DEVICE_ATTR(raw_data, 0664, hx9035_raw_data_show, NULL);
static DEVICE_ATTR(reg_write,  0664, NULL, hx9035_register_write_store);
static DEVICE_ATTR(reg_read, 0664, NULL, hx9035_register_read_store);
static DEVICE_ATTR(channel_en, 0664, hx9035_channel_en_show, hx9035_channel_en_store);
static DEVICE_ATTR(calibrate, 0664, hx9035_manual_offset_calibration_show, hx9035_manual_offset_calibration_store);
static DEVICE_ATTR(prox_state, 0664, hx9035_prox_state_show, NULL);
static DEVICE_ATTR(polling_period, 0664, hx9035_polling_show, hx9035_polling_store);
static DEVICE_ATTR(loglevel, 0664, hx9035_loglevel_show, hx9035_loglevel_store);
static DEVICE_ATTR(accuracy, 0664, hx9035_accuracy_show, hx9035_accuracy_store);
static DEVICE_ATTR(output_switch, 0664, hx9035_output_switch_show, hx9035_output_switch_store);
static DEVICE_ATTR(dump, 0664, hx9035_dump_show, NULL);
static DEVICE_ATTR(offset_dac, 0664, hx9035_offset_dac_show, NULL);
static DEVICE_ATTR(offset_csn, 0664, hx9035_offset_csn_show, NULL);
static DEVICE_ATTR(reinitialize,  0664, NULL, hx9035_reinitialize_store);
static DEVICE_ATTR(hw_monitor_en, 0664, hx9035_hw_monitor_en_show, hx9035_hw_monitor_en_store);
static DEVICE_ATTR(threshold, 0664, hx9035_threshold1_show, hx9035_threshold1_store);
static DEVICE_ATTR(thres_alg, 0664, hx9035_thres_alg_show, hx9035_thres_alg_store);

static struct attribute *hx9035_class_attrs[] = {
    &dev_attr_raw_data.attr,
    &dev_attr_reg_write.attr,
    &dev_attr_reg_read.attr,
    &dev_attr_channel_en.attr,
    &dev_attr_calibrate.attr,
    &dev_attr_prox_state.attr,
    &dev_attr_polling_period.attr,
    &dev_attr_threshold.attr,
    &dev_attr_loglevel.attr,
    &dev_attr_accuracy.attr,
    &dev_attr_output_switch.attr,
    &dev_attr_dump.attr,
    &dev_attr_offset_dac.attr,
    &dev_attr_offset_csn.attr,
    &dev_attr_reinitialize.attr,
    &dev_attr_hw_monitor_en.attr,
    &dev_attr_thres_alg.attr,
    NULL,
};

struct attribute_group hx9035_class = {
        .attrs = hx9035_class_attrs,
...
static ssize_t ch0_cap_diff_dump_show(const struct class *class,
...
static ssize_t ch2_cap_diff_dump_show(const struct class *class,
                                      const struct class_attribute *attr,
                                      char *buf)
{
    ssize_t len = 0;
...
static ssize_t ch3_cap_diff_dump_show(const struct class *class,
                                      const struct class_attribute *attr,
                                      char *buf)
{
    ssize_t len = 0;
...
#if POWER_ENABLE
static ssize_t power_enable_show(const struct class *class,
                                 const struct class_attribute *attr,
                                 char *buf)
{
    return sprintf(buf, "%d\n", hx9035_pdata.power_state); 
}
static ssize_t power_enable_store(const struct class *class,
                                  const struct class_attribute *attr,
                                  const char *buf, size_t count)
{
    int ret = -1;
...
static ssize_t user_test_show(const struct class *class,
                                 const struct class_attribute *attr,
                                 char *buf)
{
    return sprintf(buf, "%d\n", hx9035_pdata.user_test);
}
static ssize_t user_test_store(const struct class *class,
                                  const struct class_attribute *attr,
                                  const char *buf, size_t count)
{
    int ret = -1;
...
static int hx9035_probe(struct i2c_client *client)
{
    int type = 0;
    int ii = 0;
    int ret = 0;
...
//}end =============================================================
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9035_channels); ii++) {
        if ((hx9035_pdata.channel_used_flag >> ii) & 0x1) { //used flag的值来自dts，表示通道有效性，每一位表示一个通道。
            ...
            hx9035_channels[ii].hx9035_sensors_classdev.name = "HX9035";
    ...
    //debug sys fs
    ret = sysfs_create_group(&client->dev.kobj, &hx9035_class);
    if (ret)
    {
        PRINT_ERR("Failed to create sysfs node.");
    ...
    if (ret < 0) {
        PRINT_ERR("register capsense class failed (%s)\n", capsense_class.name);
        return ret;
    }
...
failed_class_register:
    ...
    sysfs_remove_group(&client->dev.kobj, &hx9035_class);
...
static void hx9035_remove(struct i2c_client *client)
{
    struct hx9035_channel_info *channel_p = NULL;
    int ii = 0;
