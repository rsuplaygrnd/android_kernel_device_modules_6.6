...
#include <linux/A96T376.h> /* main struct, interrupt,init,pointers */
...
static void ForcetoTouched(pabovXX_t this)
{
	...
	if (this && pDevice) {
		...
		if (mEnabled) {
			input_report_rel(input_ch0, REL_MISC, CAP_STATE_NEAR);
			input_report_rel(input_ch0, REL_X, 2);
			input_sync(input_ch0);
			input_report_rel(input_ch1, REL_MISC, CAP_STATE_NEAR);
			input_report_rel(input_ch1, REL_X, 2);
			input_sync(input_ch1);
#ifndef CONFIG_INPUT_ABOV_TWO_CHANNEL
			input_report_rel(input_ch2, REL_MISC, CAP_STATE_NEAR);
            ...
            if (this->power_state) {
                if ((capsense_state & pCurrentButton->mask) == (pCurrentButton->mask & ABOV_CAP_BUTTON_MASK)) {
                    if ((board->cap_channel_ch0 == counter) &&  (this->channel_status & 0x01)) {
                        input_report_rel(input_ch0, REL_MISC, CAP_STATE_FAR);
						input_report_rel(input_ch0, REL_X, 2);
                        input_sync(input_ch0);
                    } else if ((board->cap_channel_ch1 == counter) &&  (this->channel_status & 0x02)) {
                        input_report_rel(input_ch1, REL_MISC, CAP_STATE_FAR);
						input_report_rel(input_ch1, REL_X, 2);
                        input_sync(input_ch1);
                    } else if ((board->cap_channel_ch2 == counter) &&  (this->channel_status & 0x04)) {
                        input_report_rel(input_ch2, REL_MISC, CAP_STATE_FAR);
						input_report_rel(input_ch2, REL_X, 2);
                        input_sync(input_ch2);
                    }
			    ...
			    case IDLE: /* Button is being in far state! */
				    if ((capsense_state & pCurrentButton->mask) == pCurrentButton->mask) {
					    printk("ABOV CH%d State=BODY.\n", counter);
					    if (board->cap_channel_ch0 == counter) {
					    	input_report_rel(input_ch0, REL_MISC, CAP_STATE_BODY);
                            input_report_rel(input_ch0, REL_X, 2);
						    input_sync(input_ch0);
					    } else if (board->cap_channel_ch1 == counter) {
						    input_report_rel(input_ch1, REL_MISC, CAP_STATE_BODY);
                            input_report_rel(input_ch1, REL_X, 2);
    						input_sync(input_ch1);
	    				}
#ifndef CONFIG_INPUT_ABOV_TWO_CHANNEL
					    else if (board->cap_channel_ch2 == counter) {
						    input_report_rel(input_ch2, REL_MISC, CAP_STATE_BODY);
                            input_report_rel(input_ch2, REL_X, 2);
    						input_sync(input_ch2);
	    				}
#endif
					    pCurrentButton->state = S_BODY;
				    } else if ((capsense_state & pCurrentButton->mask) == (pCurrentButton->mask & ABOV_CAP_BUTTON_MASK)) {
			    ...
			    case S_BODY: /* Button is being in 0mm! */
				    if ((capsense_state & pCurrentButton->mask) == pCurrentButton->mask) {
					    printk("ABOV CH%d still in BODY State.\n", counter);
    				} else if ((capsense_state & pCurrentButton->mask) == (pCurrentButton->mask & ABOV_CAP_BUTTON_MASK)) {
	    				printk("ABOV CH%d State=NEAR.\n", counter);
		    			if ((board->cap_channel_ch0 == counter) &&  (this->channel_status & 0x01)) {
			    			input_report_rel(input_ch0, REL_MISC, CAP_STATE_NEAR);
		...
		pabov_platform_data_t pplatData)
{
	...
	int ret;
	//bug676569,jiaoyuxin.wt,add,20210715,distinguish sar firmware according to Latin America version
    client->irq = of_get_named_gpio(np, "abov,irq-gpio-std", 0);
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "1", 1) && (mEnabled == 0)) {
		...
		input_report_rel(input_ch0, REL_MISC, CAP_STATE_NONE);
		input_report_rel(input_ch0, REL_X, 2);
		input_sync(input_ch0);
		input_report_rel(input_ch1, REL_MISC, CAP_STATE_NONE);
		input_report_rel(input_ch1, REL_X, 2);
		input_sync(input_ch1);
#ifndef CONFIG_INPUT_ABOV_TWO_CHANNEL
		input_report_rel(input_ch2, REL_MISC, CAP_STATE_NONE);
		input_report_rel(input_ch2, REL_X, 2);
		input_sync(input_ch2);
#endif
...
static ssize_t update_fw_show(const struct class *class,
		const struct class_attribute *attr,
		char *buf)
{
	u8 fw_version = 0;
	...
	return snprintf(buf, 37, "ABOV_CapSensor_FW_V:0x%02x\n", fw_version);
}

static ssize_t update_fw_store(const struct class *class,
		const struct class_attribute *attr,
		const char *buf, size_t count)
{
	pabovXX_t this = abov_sar_ptr;
...
static ssize_t set_sales_code_store(const struct class *class,
		const struct class_attribute *attr,
		const char *buf, size_t count)
{
	LOG_INFO("%s: buf = %s\n",__func__, buf);
	if(!strncmp(buf, "XAC",SALE_CODE_STR_LENGTH + 1)){
		is_anfr_sar = true;
	}
	return count;
}

static ssize_t force_update_fw_show(const struct class *class,
		const struct class_attribute *attr,
		char *buf)
{
	u8 fw_version = 0;
...
static ssize_t force_update_fw_store(const struct class *class,
		const struct class_attribute *attr,
		const char *buf, size_t count)
{
	pabovXX_t this = abov_sar_ptr;
...
static ssize_t bl_show(const struct class *class, const struct class_attribute *attr, char *buf)
{
    u8 val = 0;
    u16 temp = 0;
    char *p = buf;
    ...
    for (size_t index = 0; index <= 14; index++) {
        ...
        if (0 == (index + 1) % 2) {
            temp |= val;
            buf += sprintf(buf, "ch%zu_bl=%d;", index / 2, temp);
            temp = 0;
        } else
            temp |= val << 8;
    }
...
#if POWER_ENABLE
static ssize_t power_enable_show(const struct class *class, const struct class_attribute *attr, char *buf)
{
    pabovXX_t this = abov_sar_ptr;
    return sprintf(buf, "%d\n", this->power_state);
}
static ssize_t power_enable_store(const struct class *class, const struct class_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    pabovXX_t this = abov_sar_ptr;
...
static struct class_attribute class_attr_set_sales_code = __ATTR(set_sales_code, 0644, NULL, set_sales_code_store);
#if POWER_ENABLE
...
static struct attribute *abov_capsense_attrs[] = {
    ...
    &class_attr_set_sales_code.attr,
	NULL,
};
...
err_ch2_register:
	LOG_ERR("%s input free input_ch2 device.\n", __func__);
...
err_ch1_register:
	LOG_ERR("%s input free input_ch1 device.\n", __func__);
...
err_ch0_register:
	LOG_ERR("%s input free input_ch0 device.\n", __func__);
 ...
 */
static void abov_remove(struct i2c_client *client)
{
	pabov_platform_data_t pplatData = 0;
	pabov_t pDevice = 0;
