        ...
        struct device_attribute *attr, char *buf)
{
    ...
    for(ph =0; ph < NUM_PHASES; ph++)
    {
        ...
        count = sprintf(buf+count, "PH = %d", offset);
    }
    ...
    for (i = 0; i < ARRAY_SIZE(smtc_phase_table); i++) {
        if (!strcmp(temp_input_dev->name, smtc_phase_table[i].name)) {
            if (enable) {
                ...
                pr_info("smtc:sx9338:ph%zu: enable, now psmtcButtons value is 0X%X\n", i, val);
            } else {
                ...
                /* +S96818AA1-6209, duxin2.wt,ADD, 2023/06/09, wt Adaptive sensor new hal */
                input_sync(smtc_phase_table[i].input);
                pr_info("smtc:sx9338:ph%zu: disable, now psmtcButtons value is 0X%X\n", i, val);
            }
        }
    }
    return count;
...
static ssize_t ch4_cap_diff_dump_show(const struct class *class,
                                      const struct class_attribute *attr,
                                      char *buf)
{
...
static int smtc_parse_dts(Self self)
{
    ...
    self->gpio = of_get_named_gpio(dNode, "Semtech,nirq-gpio", 0);
...
//#################################################################################################
static int smtc_probe(struct i2c_client *client)
{    
    int ret = 0, i;
    Self self = NULL;
    ...
    if (!self)
    {
        SMTC_LOG_ERR("Failed to create self. memory size=%lu bytes.", sizeof(self_t));
        return -ENOMEM;
    }
...
/* +S96818AA1-1936, liuling3.wt,ADD, 2023/05/29, wt Adaptive sensor hal */
    ...
    if (ret < 0) {
        SMTC_LOG_ERR("register capsense class failed (%s)\n", capsense_class.name);
        return ret;
    }
/* -S96818AA1-1936, liuling3.wt,ADD, 2023/05/29, wt Adaptive sensor hal */
...
//#################################################################################################
static void smtc_remove(struct i2c_client *client)
{
    int ret, i;
