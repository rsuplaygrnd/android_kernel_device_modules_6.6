...
#if IS_ENABLED(CONFIG_W2_CHARGER_PRIVATE) || IS_ENABLED(CONFIG_N28_CHARGER_PRIVATE)
...
// S96818AA3-815, gujiayin.wt, MODIFY, 20240918, Change the range for the battery temperature to 70 degrees
		tbatt_value = 70;
	} else {
...
void wt_fgauge_get_profile_id(struct platform_device *battery_dev,
	struct mtk_battery *gm)
{
	int id_volt = 0;
	int id = 0;
	...
	channel = devm_iio_channel_get(&(battery_dev->dev), "batteryID-channel");
	if (IS_ERR(channel)) {
		ret = PTR_ERR(channel);
	...
	bm_err(gm, "[%s]battery_id_voltage is %d\n", __func__, id_volt);
	...
	bm_err(gm, "[%s][Loren]Battery id (%d)\n",
		__func__,
		gm->battery_id);
}
#else
...
#if defined (CONFIG_N28_CHARGER_PRIVATE) || defined (CONFIG_W2_CHARGER_PRIVATE)
static int wt_get_batt_cycle(struct mtk_battery_manager *bm)
{
	...
	if (bm->gm1 != NULL) {
		if(!bm->gm1->bat_plug_out) {
			if (bm->gm1->battery_id < 0 || bm->gm1->battery_id >= TOTAL_BATTERY_NUMBER)
				ret_val = bm->gm1->fg_table_cust_data.fg_profile[0].q_max * 10;
			else
				ret_val = bm->gm1->fg_table_cust_data.fg_profile[bm->gm1->battery_id].q_max * 10;
			cycle += (bm->gm1->bat_cycle + 1) * ret_val;
			qmax += ret_val;
		}
	}
	...
	if (bm->gm2 != NULL) {
		if(!bm->gm2->bat_plug_out) {
			if (bm->gm2->battery_id < 0 || bm->gm2->battery_id >= TOTAL_BATTERY_NUMBER)
				ret_val = bm->gm2->fg_table_cust_data.fg_profile[0].q_max * 10;
			else
				ret_val = bm->gm2->fg_table_cust_data.fg_profile[bm->gm2->battery_id].q_max * 10;
			cycle += (bm->gm2->bat_cycle + 1) * ret_val;
			qmax += ret_val;;
		}
	}
...
int wt_set_batt_cycle_fv(void)
{
	...
	if (IS_ERR_OR_NULL(gm->bm))
		batt_cycle = 1;
	else
		batt_cycle = wt_get_batt_cycle(gm->bm);

	if (gm->batt_cycle_fv_cfg == NULL) {
		bm_err(gm, "[%s] gm->batt_cycle_fv_cfg is NULL\n", __func__);
		return 0;
	}

	if (((cycle == batt_cycle) && (temp_cycle == -1)) || ((cycle == temp_cycle) && (temp_cycle != -1))) {
		return cycle_fv;
	}

	if(temp_cycle != -1) {
		cycle = temp_cycle;
		bm_err(gm, "bat_cycle=%d,temp_cycle=%d,temp_cycle is effective\n",batt_cycle,cycle);
	} else if (batt_cycle >= 0) {
		cycle = batt_cycle;
		bm_err(gm, "bat cycle = %d is effective\n",cycle);
	}
	...
	if (gm->batt_cycle_fv_cfg && gm->fv_levels) {
		for (i = 0; i < gm->fv_levels; i += 3) {
			if ((cycle >= gm->batt_cycle_fv_cfg[i]) && (cycle <= gm->batt_cycle_fv_cfg[i + 1])) {
				...
				bm_err(gm, "WT set cv = %d,cycle=%d,i=%d\n", cycle_fv,cycle,i);
				return cycle_fv;
			}
		}
		if (i >= gm->fv_levels){
			cycle_fv = gm->batt_cycle_fv_cfg[gm->fv_levels - 1];
			bm_err(gm, "the maximum cycle level WT set cv = %d,cycle=%d,i=%d,fv_levels=%d\n", cycle_fv,cycle,i,gm->fv_levels);
			return cycle_fv;
		}
	}
	return 0;
	...
	struct mtk_battery *gm)
{
	...
	if (of_find_property(np, "wt,batt-cycle-ranges", &byte_len)) {
		...
		if (gm->batt_cycle_fv_cfg) {
			...
			ret = of_property_read_u32_array(np, "wt,batt-cycle-ranges",
				gm->batt_cycle_fv_cfg, gm->fv_levels);
			if (ret < 0) {
				bm_err(gm, "Couldn't read battery limits ret = %d\n", ret);
				gm->batt_cycle_fv_cfg = NULL;
			} else {
				for (i = 0; i < gm->fv_levels; i += 3) {
					bm_err(gm, "wt,batt-cycle-ranges:%d,%d,%d\n",
						gm->batt_cycle_fv_cfg[i],
						gm->batt_cycle_fv_cfg[i+1],
						gm->batt_cycle_fv_cfg[i+2]);
				}
			}
		}
	} else {
		bm_err(gm, "wt,batt-cycle-ranges No configuration\n");
	}
...
int get_bootmode_from_battery(void)
{
	printk("%s:bootmode:0x%x\n", __func__, gmb->bootmode);

	return gmb->bootmode;
}
EXPORT_SYMBOL(get_bootmode_from_battery);
...
#if defined (CONFIG_W2_CHARGER_PRIVATE) || defined (CONFIG_N28_CHARGER_PRIVATE)
	wt_fgauge_get_profile_id(pdev, gm);
#else
