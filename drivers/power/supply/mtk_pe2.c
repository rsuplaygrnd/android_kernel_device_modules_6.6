...
extern bool is_mtk_pe2_probe_ok;
#endif
...
static void pe2_set_current_config(struct chg_alg_device *alg)
{
	struct mtk_pe20 *pe2 = dev_get_drvdata(&alg->dev);

	pe2_err("%s set:%d,%d,%d,%d,%d\n", __func__, pe2->input_current_limit1, pe2->charging_current_limit1,
		pe2->input_current_limit2, pe2->charging_current_limit2, pe2->ref_vbat);

	if (pe2->state == PE2_HW_READY) {
