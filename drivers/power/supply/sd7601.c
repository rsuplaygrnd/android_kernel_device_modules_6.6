...
bool is_sd7601_chg_probe_ok = false;
EXPORT_SYMBOL(is_sd7601_chg_probe_ok);
...
#if 0//bring up
	bool attach = false;
	enum charger_type chg_type = POWER_SUPPLY_TYPE_UNKNOWN;
...
static int sd7601_chg_set_usbsw(struct sd7601_chip *chip, enum sd7601_usbsw_state usbsw)
{
    struct phy *phy;
    int ret, mode = (usbsw == SD7601_USBSW_CHG) ? PHY_MODE_BC11_SET :
                           PHY_MODE_BC11_CLR;
...
static void sd7601_set_usbsw_state(struct sd7601_chip *chip, int state)
{
	...
	if (state == SD7601_USBSW_CHG)
		sd7601_chg_set_usbsw(chip, SD7601_USBSW_CHG);
		//Charger_Detect_Init();
	else
		sd7601_chg_set_usbsw(chip, SD7601_USBSW_USB);
		//Charger_Detect_Release();
}

static void sd7601_get_charger_type(struct sd7601_chip *chip)
{
	int32_t ret = 0;
	...
	if ((chip->psy_desc.type == POWER_SUPPLY_TYPE_USB) || (chip->psy_desc.type == POWER_SUPPLY_TYPE_USB_CDP)) {
		//Charger_Detect_Release();
		sd7601_chg_set_usbsw(chip, SD7601_USBSW_USB);
	}
}
...
/* S96818AA3 gujiayin.wt,modify,2024/08/21 get vbus start */
struct iio_channel ** sd7601_get_ext_channels(struct device *dev,
		 const char *const *channel_map, int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (IS_ERR(iio_ch_ext[i])) {
			...
			if (rc != -EPROBE_DEFER)
				sd7601_dbg("%s channel unavailable, %d\n", channel_map[i], rc);
			return ERR_PTR(rc);
		}
	}
...
static bool sd7601_is_pmic_chan_valid(struct sd7601_chip *chip,
		enum pmic_iio_channels chan)
{
	int rc;
	struct iio_channel **iio_list;

	if (!chip->pmic_ext_iio_chans) {
		iio_list = sd7601_get_ext_channels(chip->dev, pmic_iio_chan_name,
		ARRAY_SIZE(pmic_iio_chan_name));
		if (IS_ERR(iio_list)) {
			rc = PTR_ERR(iio_list);
			if (rc != -EPROBE_DEFER) {
				sd7601_dbg("Failed to get channels, %d\n", rc);
				chip->pmic_ext_iio_chans = NULL;
			}
			return false;
		}
		chip->pmic_ext_iio_chans = iio_list;
	}

	return true;
}

static int32_t sd7601_get_vbus_value(struct sd7601_chip *chip,  int32_t *val)
{
	int32_t ret, temp;
	struct iio_channel *iio_chan_list;

	if (!sd7601_is_pmic_chan_valid(chip, VBUS_VOLTAGE)) {
		sd7601_dbg("read vbus_dect channel fail\n");
		return -ENODEV;
	}

	iio_chan_list = chip->pmic_ext_iio_chans[VBUS_VOLTAGE];
	ret = iio_read_channel_processed(iio_chan_list, &temp);
	if (ret < 0) {
		sd7601_dbg("read vbus_dect channel fail, ret=%d\n", ret);
		return ret;
	}
	//*val = temp / 100;
	*val = temp * 369 / 39;
	sd7601_dbg("%s: vbus_volt = %d \n", __func__, *val);
...
/* S96818AA3 gujiayin.wt,modify,2024/08/21 get vbus start */
	...
	ret = sd7601_get_vbus_value(chip, &val);
	if (ret < 0) {
		sd7601_dbg("sd7601_get_vbus error \n");
		val = 0;
	}
	*vbus = val * 1000;
//#else //N28 kernel-4.19
#if 0
...
static ssize_t store_registers(struct device *dev,struct device_attribute *attr,const char *buf, size_t size)
{
	...
	if (ret)
		pr_err("SD7601 Invalid input value\n");
	//read
	if(3 == size)
	{
...
static int32_t sd7601_get_prop(struct power_supply *psy,enum power_supply_property psp,union power_supply_propval *val)
{
	...
	switch (psp) 
	{
		...
		case POWER_SUPPLY_PROP_TYPE:
			//if (chip->psy_desc.type == POWER_SUPPLY_TYPE_UNKNOWN) {
			//	sd7601_get_charger_type(chip);
			//}
...
static int sd7601_enable_vbus(struct regulator_dev *rdev)
{
	struct sd7601_chip *chip = charger_get_data(s_chg_dev_otg);
	int ret = 0;

	sd7601_dbg("%s enter\n", __func__);
	/*we should ensure that the powerpath is enabled before enable OTG*/
	ret = sd7601_enable_hiz(chip->chg_dev, false);
	if (ret) {
		sd7601_dbg("%s exit hiz failed\r\n", __func__);
	}
	ret = sd7601_set_otg_en(chip, true);
	if (ret < 0) {
		sd7601_dbg("set otg enable failed(%d)\n", ret);
		return ret;
	}
...
static int sd7601_disable_vbus(struct regulator_dev *rdev)
{
	struct sd7601_chip *chip = charger_get_data(s_chg_dev_otg);
	int ret = 0;

	sd7601_dbg("%s enter\n", __func__);
	ret = sd7601_set_otg_en(chip, false);
	if (ret < 0) {
		sd7601_dbg("set otg disable failed(%d)\n", ret);
		return ret;
	}
...
static int sd7601_is_enabled_vbus(struct regulator_dev *rdev)
{
	struct sd7601_chip *chip = charger_get_data(s_chg_dev_otg);
	uint8_t data_reg = 0;
	int32_t ret = 0;
	...
	sd7601_dbg("%s enable vbus val:%d\n", __func__, data_reg);

	return !!data_reg;
}

static struct regulator_ops sd7601_vbus_ops = {
	.enable = sd7601_enable_vbus,
	.disable = sd7601_disable_vbus,
	.is_enabled = sd7601_is_enabled_vbus,
};

static const struct regulator_desc sd7601_otg_rdesc = {
...
static int sd7601_vbus_regulator_register(struct charger_device *chg_dev)
{
	...
	chip = dev_get_drvdata(&chg_dev->dev);
	config.dev = chip->dev;
	config.driver_data = chip;
	chip->otg_rdev =
		devm_regulator_register(chip->dev, &sd7601_otg_rdesc, &config);
	chip->otg_rdev->constraints->valid_ops_mask |= REGULATOR_CHANGE_STATUS;

	if (IS_ERR(chip->otg_rdev)) {
		ret = PTR_ERR(chip->otg_rdev);
		sd7601_dbg("%s: register otg regulator failed (%d)\n", __func__, ret);
	}
	return ret;
}
/* S96818AA3 gujiayin.wt,modify,2024/08/22 otg bring up end */

static int32_t sd7601_driver_probe(struct i2c_client *client)
{
	int32_t ret = 0;
	struct sd7601_chip *chip = NULL;
...
//Antaiui <AI_BSP_CHG> <hehl> <2021-12-21> modify otg end
	is_sd7601_chg_probe_ok = true;
/* S96818AA3 gujiayin.wt,modify,2024/08/22 otg bring up start */
	/* otg regulator */
	s_chg_dev_otg = chip->chg_dev;
	ret = sd7601_vbus_regulator_register(chip->chg_dev);
	if (ret)
		pr_err("failed to register upm6910_vbus_regulator: %d\n", ret);
/* S96818AA3 gujiayin.wt,modify,2024/08/22 otg bring up end */
...
static void sd7601_driver_remove(struct i2c_client *client)
{
	...
	if(chip->irq >= 0)
		disable_irq(chip->irq);
	if(-1 != chip->intr_gpio)
#if 0 //(LINUX_VERSION_CODE < KERNEL_VERSION(6, 0, 0))
		devm_gpio_free(chip->dev,chip->intr_gpio);
#else
		gpio_free(chip->intr_gpio);
#endif
...
MODULE_DEVICE_TABLE(of, sd7601_of_match);

static const struct i2c_device_id sd7601_i2c_id[] = { 
	{"sd7601d",  0},
	{},
};
MODULE_DEVICE_TABLE(i2c, sd7601_i2c_id);
