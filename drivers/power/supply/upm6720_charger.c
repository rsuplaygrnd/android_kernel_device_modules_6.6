...
static int upm6720_enable_charge(struct upm6720 *upm, bool enable)
{
	...
	return upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_CHG_EN_MASK, val);
}
...
static int upm6720_set_busocp_th(struct upm6720 *upm, chg_mode_t mode, int threshold)
{
	...
	return upm6720_update_bits(upm, UPM6720_REG_08, UPM6720_BUS_OCP_MASK, val);
}
...
static int upm6720_parse_dt(struct upm6720 *upm, struct device *dev)
{
	...
	upm->cfg->bat_ovp_disable = of_property_read_bool(np, "upm6720,bat-ovp-disable");
	upm->cfg->bat_ocp_disable = of_property_read_bool(np, "upm6720,bat-ocp-disable");
	upm->cfg->bus_ucp_disable = of_property_read_bool(np, "upm6720,bus-ucp-disable");
	upm->cfg->bus_rcp_disable = of_property_read_bool(np, "upm6720,bus-rcp-disable");
	upm->cfg->vout_ovp_disable = of_property_read_bool(np, "upm6720,vout-ovp-disable");
	upm->cfg->tdie_flt_disable = of_property_read_bool(np, "upm6720,tdie-flt-disable");
	upm->cfg->tsbus_flt_disable = of_property_read_bool(np, "upm6720,tsbus-flt-disable");
	upm->cfg->tsbat_flt_disable = of_property_read_bool(np, "upm6720,tsbat-flt-disable");
	upm->cfg->wdt_disable = of_property_read_bool(np, "upm6720,wdt-disable");
	upm->cfg->vbus_errhi_disable = of_property_read_bool(np, "upm6720,vbus-errhi-disable");

	ret = of_property_read_u32(np, "upm6720,bat-ovp-threshold", &upm->cfg->bat_ovp_th);
	if (ret) {
		upm_err("failed to read bat-ovp-threshold\n");
		return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bat-ocp-threshold", &upm->cfg->bat_ocp_th);
	if (ret) {
	...
	ret = of_property_read_u32(np, "upm6720,bus-ovp-threshold", &upm->cfg->bus_ovp_th);
	if (ret) {
		upm_err("failed to read bus-ovp-threshold\n");
		return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bus-ocp-threshold", &upm->cfg->bus_ocp_th);
	if (ret) {
		upm_err("failed to read bus-ocp-threshold\n");
		return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bus-ucp-threshold", &upm->cfg->bus_ucp_th);
	if (ret) {
		upm_err("failed to read bus-ucp-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bus-rcp-threshold", &upm->cfg->bus_rcp_th);
	if (ret) {
		upm_err("failed to read bus-rcp-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,vac1-ovp-threshold", &upm->cfg->vac1_ovp_th);
	if (ret) {
		upm_err("failed to read vac1-ovp-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,vac2-ovp-threshold", &upm->cfg->vac2_ovp_th);
	if (ret) {
		upm_err("failed to read vac2-ovp-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,vout-ovp-threshold", &upm->cfg->vout_ovp_th);
	if (ret) {
		upm_err("failed to read vout-ovp-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,tdie-flt-threshold", &upm->cfg->tdie_flt_th);
	if (ret) {
		upm_err("failed to read tdie-flt-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,tsbus-flt-threshold", &upm->cfg->tsbus_flt_th);
	if (ret) {
		upm_err("failed to read tsbus-flt-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,tsbat-flt-threshold", &upm->cfg->tsbat_flt_th);
	if (ret) {
		upm_err("failed to read tsbat-flt-threshold\n");
		//return ret;
	}

	upm->cfg->bat_ovp_mask = of_property_read_bool(np, "upm6720,bat-ovp-mask");
	upm->cfg->bat_ocp_mask = of_property_read_bool(np, "upm6720,bat-ocp-mask");
	upm->cfg->bus_ovp_mask = of_property_read_bool(np, "upm6720,bus-ovp-mask");
	upm->cfg->bus_ocp_mask = of_property_read_bool(np, "upm6720,bus-ocp-mask");
	upm->cfg->bus_ucp_mask = of_property_read_bool(np, "upm6720,bus-ucp-mask");
	upm->cfg->bus_rcp_mask = of_property_read_bool(np, "upm6720,bus-rcp-mask");
	upm->cfg->vout_ovp_mask = of_property_read_bool(np, "upm6720,vout-ovp-mask");
	upm->cfg->vac1_ovp_mask = of_property_read_bool(np, "upm6720,vac1-ovp-mask");
	upm->cfg->vac2_ovp_mask = of_property_read_bool(np, "upm6720,vac2-ovp-mask");

	upm->cfg->vout_present_mask = of_property_read_bool(np, "upm6720,vout-present-mask");
	upm->cfg->vac1_present_mask = of_property_read_bool(np, "upm6720,vac1-present-mask");
	upm->cfg->vac2_present_mask = of_property_read_bool(np, "upm6720,vac2-present-mask");
	upm->cfg->vbus_present_mask = of_property_read_bool(np, "upm6720,vbus-present-mask");
	upm->cfg->acrb1_config_mask = of_property_read_bool(np, "upm6720,acrb1-config-mask");
	upm->cfg->acrb2_config_mask = of_property_read_bool(np, "upm6720,acrb2-config-mask");
	upm->cfg->cfly_short_mask = of_property_read_bool(np, "upm6720,cfly-short-mask");
	upm->cfg->adc_done_mask = of_property_read_bool(np, "upm6720,adc-done-mask");
	upm->cfg->ss_timeout_mask = of_property_read_bool(np, "upm6720,ss-timeout-mask");
	upm->cfg->tsbus_flt_mask = of_property_read_bool(np, "upm6720,tsbus-flt-mask");
	upm->cfg->tsbat_flt_mask = of_property_read_bool(np, "upm6720,tsbat-flt-mask");
	upm->cfg->tdie_flt_mask = of_property_read_bool(np, "upm6720,tdie-flt-mask");
	upm->cfg->wd_mask = of_property_read_bool(np, "upm6720,wd-mask");
	upm->cfg->regn_good_mask = of_property_read_bool(np, "upm6720,regn-good-mask");
	upm->cfg->conv_active_mask = of_property_read_bool(np, "upm6720,conv-active-mask");
	upm->cfg->vbus_errhi_mask = of_property_read_bool(np, "upm6720,vbus-errhi-mask");

	upm->cfg->bat_ovp_alm_disable = of_property_read_bool(np, "upm6720,bat-ovp-alm-disable");
	upm->cfg->bat_ocp_alm_disable = of_property_read_bool(np, "upm6720,bat-ocp-alm-disable");
	upm->cfg->bat_ucp_alm_disable = of_property_read_bool(np, "upm6720,bat-ucp-alm-disable");
	upm->cfg->bus_ovp_alm_disable = of_property_read_bool(np, "upm6720,bus-ovp-alm-disable");
	upm->cfg->tdie_alm_disable = of_property_read_bool(np, "upm6720,tdie-alm-disable");

	ret = of_property_read_u32(np, "upm6720,bat-ovp-alm-threshold", &upm->cfg->bat_ovp_alm_th);
	if (ret) {
		upm_err("failed to read bat-ovp-alm-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bat-ocp-alm-threshold", &upm->cfg->bat_ocp_alm_th);
	if (ret) {
		upm_err("failed to read bat-ocp-alm-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bat-ucp-alm-threshold", &upm->cfg->bat_ucp_alm_th);
	if (ret) {
		upm_err("failed to read bat-ucp-alm-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bus-ovp-alm-threshold", &upm->cfg->bus_ovp_alm_th);
	if (ret) {
		upm_err("failed to read bus-ovp-alm-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,bus-ocp-alm-threshold", &upm->cfg->bus_ocp_alm_th);
	if (ret) {
		upm_err("failed to read bus-ocp-alm-threshold\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,tdie-alm-threshold", &upm->cfg->tdie_alm_th);
	if (ret) {
		upm_err("failed to read tdie-alm-threshold\n");
		//return ret;
	}

	upm->cfg->bat_ovp_alm_mask = of_property_read_bool(np, "upm6720,bat-ovp-alm-mask");
	upm->cfg->bat_ocp_alm_mask = of_property_read_bool(np, "upm6720,bat-ocp-alm-mask");
	upm->cfg->bat_ucp_alm_mask = of_property_read_bool(np, "upm6720,bat-ucp-alm-mask");
	upm->cfg->bus_ovp_alm_mask = of_property_read_bool(np, "upm6720,bus-ovp-alm-mask");
	upm->cfg->bus_ocp_alm_mask = of_property_read_bool(np, "upm6720,bus-ocp-alm-mask");
	upm->cfg->tsbus_tsbat_alm_mask = of_property_read_bool(np, "upm6720,tsbus-tsbat-alm-mask");
	upm->cfg->tdie_alm_mask = of_property_read_bool(np, "upm6720,tdie-alm-mask");

	upm->cfg->bus_pd_en = of_property_read_bool(np, "upm6720,bus-pulldown-en");
	upm->cfg->vac1_pd_en = of_property_read_bool(np, "upm6720,vac1-pulldown-en");
	upm->cfg->vac2_pd_en = of_property_read_bool(np, "upm6720,vac2-pulldown-en");

	ret = of_property_read_u32(np, "upm6720,sense-resistor-mohm", &upm->cfg->sense_r_mohm);
	if (ret) {
		upm_err("failed to read sense-resistor-mohm\n");
		return ret;
	}

	ret = of_property_read_u32(np, "upm6720,ss-timeout", &upm->cfg->ss_timeout);
	if (ret) {
		upm_err("failed to read ss-timeout\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,wdt-set", &upm->cfg->wdt_set);
	if (ret) {
		upm_err("failed to read wdt-set\n");
		//return ret;
	}

	upm->cfg->chg_config_1 = of_property_read_bool(np, "upm6720,chg-config-1");

	ret = of_property_read_u32(np, "upm6720,fsw-set", &upm->cfg->fsw_set);
	if (ret) {
		upm_err("failed to read fsw-set\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,freq-shift", &upm->cfg->freq_shift);
	if (ret) {
		upm_err("failed to read freq-shift\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,ibus-ucp-fall-dg-sel", &upm->cfg->ibus_ucp_fall_dg_sel);
	if (ret) {
		upm_err("failed to read ibus-ucp-fall-dg-sel\n");
		//return ret;
	}

	upm->cfg->adc_enable = of_property_read_bool(np, "upm6720,adc-enable");

	ret = of_property_read_u32(np, "upm6720,adc-rate", &upm->cfg->adc_rate);
	if (ret) {
		upm_err("failed to read adc-rate\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,adc-avg", &upm->cfg->adc_avg);
	if (ret) {
		upm_err("failed to read adc-avg\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,adc-avg-init", &upm->cfg->adc_avg_init);
	if (ret) {
		upm_err("failed to read adc-avg-init\n");
		//return ret;
	}

	ret = of_property_read_u32(np, "upm6720,adc-sample-bit", &upm->cfg->adc_sample);
	if (ret) {
		upm_err("failed to read adc-sample-bit\n");
		//return ret;
	}

	upm->cfg->ibus_adc_disable = of_property_read_bool(np, "upm6720,ibus-adc-disable");
	upm->cfg->vbus_adc_disable = of_property_read_bool(np, "upm6720,vbus-adc-disable");
	upm->cfg->vac1_adc_disable = of_property_read_bool(np, "upm6720,vac1-adc-disable");
	upm->cfg->vac2_adc_disable = of_property_read_bool(np, "upm6720,vac2-adc-disable");
	upm->cfg->vout_adc_disable = of_property_read_bool(np, "upm6720,vout-adc-disable");
	upm->cfg->vbat_adc_disable = of_property_read_bool(np, "upm6720,vbat-adc-disable");
	upm->cfg->ibat_adc_disable = of_property_read_bool(np, "upm6720,ibat-adc-disable");
	upm->cfg->tsbus_adc_disable = of_property_read_bool(np, "upm6720,tsbus-adc-disable");
	upm->cfg->tsbat_adc_disable = of_property_read_bool(np, "upm6720,tsbat-adc-disable");
	upm->cfg->tdie_adc_disable = of_property_read_bool(np, "upm6720,tdie-adc-disable");
...
static enum power_supply_property upm6720_charger_props[] = {
	...
	POWER_SUPPLY_PROP_AUTHENTIC,//charging enabled
	POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE,//cp vbus
	POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT,//cp ibus
	POWER_SUPPLY_PROP_VOLTAGE_NOW,//cp vbat
	POWER_SUPPLY_PROP_CURRENT_NOW,//cp ibat
	POWER_SUPPLY_PROP_TEMP_AMBIENT,//die temp
	//POWER_SUPPLY_PROP_SCOPE,//vbus status
#if 0
                ...
                union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_SCOPE:
			ret = upm6720_read_byte(upm, UPM6720_REG_05, &reg_val);
			if (!ret)
                    ...
                    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		...
		upm_info("POWER_SUPPLY_PROP_AUTHENTIC: %s\n",
...
#if 0
/* S96818AA3 gujiayin.wt,modify,2024/08/27 cp bring up start*/
static ssize_t charging_enabled_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	bool en = false;
	int ret = 0;
	struct upm6720 *upm = NULL;
	struct power_supply *psy = power_supply_get_by_name("cp-standalone");

	if (psy == NULL || IS_ERR(psy)) {
		upm_err("[%s]psy is not rdy\n", __func__);
		return -EINVAL;
	}

	upm = power_supply_get_drvdata(psy);
	if (upm == NULL) {
		upm_err("[%s]cp is not rdy\n", __func__);
		return -EINVAL;
	} else {
		ret = upm6720_check_charge_enabled(upm, &en);
		if (!ret)
...
static ssize_t charging_enabled_store(
	struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoint(buf, 10, &val) == 0) {
		upm_err("cp charging enable\n");
	} else if (sscanf(buf, "%10d\n", &val) == 1) {
		upm_err("cp charging disable\n");
	}
		upm6720_enable_charge(upm, val);
		upm6720_check_charge_enabled(upm, &upm->charge_enabled);
...
static ssize_t upm_bus_voltage_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int result = 0;
	int ret = 0;
	struct upm6720 *upm = NULL;
...
static DEVICE_ATTR_RO(upm_bus_voltage);

static ssize_t upm_bus_current_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int result = 0;
	int ret = 0;
	struct upm6720 *upm = NULL;
...
static DEVICE_ATTR_RO(upm_bus_current);

static ssize_t upm_bat_voltage_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int result = 0;
	int ret = 0;
	struct upm6720 *upm = NULL;
...
static DEVICE_ATTR_RO(upm_bat_voltage);

static ssize_t upm_bat_current_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int result = 0;
	int ret = 0;
	struct upm6720 *upm = NULL;
...
static DEVICE_ATTR_RO(upm_bat_current);

static ssize_t upm_die_temp_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int result = 0;
	int ret = 0;
	struct upm6720 *upm = NULL;
...
static DEVICE_ATTR_RO(upm_die_temp);

static ssize_t upm_vbus_status_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	u8 reg_val = 0;
	int ret = 0;
	struct upm6720 *upm = NULL;
	...
	if (upm == NULL) {
		...
		ret = upm6720_read_byte(upm, UPM6720_REG_35, &reg_val);
		if (!ret)
			upm_err("[%s]get vbus_status succ\n", __func__);
	}

	return sprintf(buf, "%d\n", (int)reg_val);
}
static DEVICE_ATTR_RO(upm_vbus_status);
/* S96818AA3 gujiayin.wt,modify,2024/08/27 cp bring up end*/
#endif
...
#if 0
	/* S96818AA3 gujiayin.wt,modify,2024/08/27 cp bring up add*/
	device_create_file(&upm->fc2_psy->dev, &dev_attr_charging_enabled);
	device_create_file(&upm->fc2_psy->dev, &dev_attr_upm_bus_voltage);
	device_create_file(&upm->fc2_psy->dev, &dev_attr_upm_bus_current);
	device_create_file(&upm->fc2_psy->dev, &dev_attr_upm_bat_voltage);
	device_create_file(&upm->fc2_psy->dev, &dev_attr_upm_bat_current);
	device_create_file(&upm->fc2_psy->dev, &dev_attr_upm_die_temp);
	device_create_file(&upm->fc2_psy->dev, &dev_attr_upm_vbus_status);
#endif
...
static void upm6720_charger_shutdown(struct i2c_client *client)
{
	...
	upm6720_enable_adc(upm, false);
	upm6720_enable_charge(upm, false);
	upm6720_set_reg_reset(upm);
	mutex_destroy(&upm->i2c_rw_lock);
}
