...
wait:
	...
	for (i = 0; i < mci->nr_port; i++) {
		...
		if (mci->bc12_sel[i] == MTK_CTD_BY_SUBPMIC_PWR_RDY)
			continue;
#if IS_ENABLED(CONFIG_MTK_CHARGER) && IS_ENABLED(CONFIG_N28_CHARGER_PRIVATE)
		charger_dev_enable_chg_type_det(mci->chg_dev[0], attach);
#endif /* CONFIG_MTK_CHARGER */
...
#endif
			dev_info(mci->dev,
				 "%s Charger plug in, polarity = %d, cc_ori = %d\n",
				 __func__, noti->typec_state.polarity, noti->typec_state.polarity + 1);
