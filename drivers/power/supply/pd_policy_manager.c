...
struct cp_charging g_cp_charging = {
	.cp_chg_status = CP_REENTER | CP_EXIT,
	.cp_jeita_cc = 5500000,
	.cp_jeita_cv = 4400000,
	.cp_therm_cc = 5500000,
	.disable_quick_charge = false,
};
EXPORT_SYMBOL(g_cp_charging);
...
*/
	ret = power_supply_get_property(pdpm->cp_psy,
			POWER_SUPPLY_PROP_TEMP_AMBIENT, &val);
	if (!ret)
...
static int usbpd_pm_enable_sw_term(struct usbpd_pm *pdpm, bool enable)
{
	int ret;
	union power_supply_propval val = {0,};
...
static int usbpd_pm_enable_sw(struct usbpd_pm *pdpm, bool enable)
{
	...
	if (enable) {
		if (g_cp_charging.disable_quick_charge) {
			val.intval = 1100000;
		} else {
			val.intval = 1600000;
		}
			...
			unsigned long event, void *data)
{
	...
	pr_err("event:%d, connected:%d\n", (int)event, noti->pd_state.connected);
...
// change cp.vbat_volt to sw.vbat_volt for entry PD_PM_STATE_FC2_ENTRY_1 state.
		...
		}  else if (g_cp_charging.disable_quick_charge) {
...
// S96818AA1-4726, churui1.wt, MODIFY, 20230516, fixed no current when pps charger is inserted for the first time after booting
		...
		usbpd_pm_enable_sw_term(pdpm, false);
	...
	case PD_PM_STATE_FC2_TUNE:
		if (g_cp_charging.disable_quick_charge) {
			pr_err("Move to switch charge, disable quick charge from UI");
			recover = true;
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
		}
		usbpd_pm_update_vbus_err_status(pdpm);
...
/* -churui1.wt, ADD, 20230602, CP charging control */
	...
	case PD_PM_STATE_FC2_EXIT:
		...
		usbpd_pm_enable_sw_term(pdpm, true);
