 ...
 * block read function
 */
static int sp2130_i2c_read_block(struct sp2130 *bq, u8 reg, u8 len, u8 *data)
{
	int ret;
...
int sp2130_shipmode_enable_adc(bool enable)
{
	if(!otg_bq)
		return -EINVAL;
	if(enable) {
		sp2130_enable_adc(otg_bq, true);
	} else {
		sp2130_enable_adc(otg_bq, false);
	}
	return 0;
}
EXPORT_SYMBOL(sp2130_shipmode_enable_adc);
...
static int sp2130_reset_reg(struct sp2130 *bq, bool enable)
{
	int ret;
	u8 val;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		bq_err("POWER_SUPPLY_PROP_CURRENT_NOW: %d\n", result);
		break;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		...
		bq_err("POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT: %d\n", result);
		break;
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		...
		sp2130_check_charge_enabled(bq, &bq->charge_enabled);
		bq_info("POWER_SUPPLY_PROP_AUTHENTIC: %s\n",
	...
	struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoint(buf, 10, &val) == 0) {
		bq_err("cp charging enable\n");
	} else if (sscanf(buf, "%10d\n", &val) == 1) {
		bq_err("cp charging disable\n");
	}
		sp2130_enable_charge(bq, val);
		sp2130_check_charge_enabled(bq, &bq->charge_enabled);
...
static ssize_t upm_bus_voltage_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (bq == NULL) {
		...
		sp2130_get_adc_data(bq, ADC_VBUS, &result);
		bq->vbus_volt = result;
	}
...
static ssize_t upm_bus_current_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (bq == NULL) {
		...
		sp2130_get_adc_data(bq, ADC_IBUS, &result);
		bq->ibus_curr = result;
...
static ssize_t upm_bat_voltage_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (bq == NULL) {
		...
		sp2130_get_adc_data(bq, ADC_VBAT, &result);
		bq->vbat_volt = result;
	}
...
static ssize_t upm_bat_current_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (bq == NULL) {
		...
		sp2130_get_adc_data(bq, ADC_IBAT, &result);
		bq->ibat_curr = result;
...
static ssize_t upm_die_temp_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (bq == NULL) {
		...
		sp2130_get_adc_data(bq, ADC_TDIE, &result);
		bq->die_temp = result;
	}
...
static int sp2130_psy_register(struct sp2130 *bq)
{
	...
	device_create_file(&bq->fc2_psy->dev, &dev_attr_charging_enabled);
	device_create_file(&bq->fc2_psy->dev, &dev_attr_upm_bus_voltage);
	device_create_file(&bq->fc2_psy->dev, &dev_attr_upm_bus_current);
	device_create_file(&bq->fc2_psy->dev, &dev_attr_upm_bat_voltage);
	device_create_file(&bq->fc2_psy->dev, &dev_attr_upm_bat_current);
	device_create_file(&bq->fc2_psy->dev, &dev_attr_upm_die_temp);
	device_create_file(&bq->fc2_psy->dev, &dev_attr_upm_vbus_status);
#endif
...
static void sp2130_check_status_flags(struct sp2130 *bq)
{
	...
	sp2130_i2c_read_block(bq, SP2130_REG_05, 2, &sf_reg[0]);
	if (1) {
	...
	sp2130_i2c_read_block(bq, SP2130_REG_09, 11, &sf_reg[2]);
	...
	sp2130_i2c_read_block(bq, SP2130_REG_2E, 9, &sf_reg[13]);
	if (1) {
...
int sp2130_set_otg_txmode(bool enable)
{
	...
	if (enable) {
		...
		ret = sp2130_write_byte(otg_bq, SP2130_REG_2F, val1 <<= SP2130_EN_OTG_SHIFT);
		if (ret < 0) {
			printk("sp2130 write OTG_EN:1 fail: %d\n", ret);
			return ret;
		}
		msleep(30);
		ret = sp2130_update_bits(otg_bq, SP2130_REG_30,SP2130_EN_ACDRV1_MASK, val2 <<= SP2130_EN_ACDRV1_SHIFT);
		if (ret < 0) {
			printk("sp2130 write ACDRV1_STAT:1 fail: %d\n", ret);
			return ret;
		}
	} else {
		val1 = SP2130_ACDRV1_DISABLE;
		val2 = SP2130_OTG_DISABLE;
		ret = sp2130_update_bits(otg_bq, SP2130_REG_30,SP2130_EN_ACDRV1_MASK, val1 <<= SP2130_EN_ACDRV1_SHIFT);
		if (ret < 0) {
			printk("sp2130 write ACDRV1_STAT:0 fail: %d\n", ret);
			return ret;
		}
		msleep(30);
		ret = sp2130_write_byte(otg_bq, SP2130_REG_2F, val2 <<= SP2130_EN_OTG_SHIFT);
		if (ret < 0) {
			printk("sp2130 write OTG_EN:0 fail: %d\n", ret);
			return ret;
		}
	}
...
static int sp2130_charger_probe(struct i2c_client *client)
{
	...
	printk("client->irq=%d", client->irq);
	ret = i2c_smbus_read_byte_data(client, SP2130_REG_31);
	if (ret != SP2130_DEVICE_ID) {
		printk("failed to communicate with chip, ret=%d\n", ret);
		return -ENODEV;
	}
	printk("bq device id=0x%x\n", ret);
...
static void sp2130_charger_remove(struct i2c_client *client)
{
	struct sp2130 *bq = i2c_get_clientdata(client);
...
static void sp2130_charger_shutdown(struct i2c_client *client)
{
	...
	sp2130_enable_adc(bq, false);
	sp2130_enable_charge(bq, false);
	sp2130_reset_reg(bq, true);
}
