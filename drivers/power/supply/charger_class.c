...
int charger_dev_set_enable_term(struct charger_device *chg_dev, bool enable)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_enable_term)
		chg_dev->ops->set_enable_term(chg_dev, enable);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_enable_term);
#endif
...
int charger_dev_set_recharge(struct charger_device *chg_dev, u32 uV)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_recharge)
		return chg_dev->ops->set_recharge(chg_dev, uV);
...
//gujiayin.wt,add shipmode ctrl satrt
...
int charger_dev_set_iinlmtsel(struct charger_device *charger_dev, bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->set_iinlmtsel)
		return charger_dev->ops->set_iinlmtsel(charger_dev, en);
