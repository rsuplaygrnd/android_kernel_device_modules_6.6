...
//	panel_notifier_call_chain(PANEL_UNPREPARE, NULL);
	...
	if (w2_gesture_status == 1) {
		printk("[FTS-lcm]txdsharp_unprepare w2_gesture_status = 1\n ");
	} else {
		printk("[FTS-lcm]txdsharp_unprepare w2_gesture_status = 0\n ");
#if 0
		ctx->reset_gpio = devm_gpiod_get(ctx->dev, "reset", GPIOD_OUT_HIGH);
		gpiod_set_value(ctx->reset_gpio, 0);
		devm_gpiod_put(ctx->dev, ctx->reset_gpio);
...
static const struct drm_display_mode default_mode = {
	...
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA_R,
	.htotal = HAC + HFP + HSA_R + HBP_R,
	.vdisplay = VAC,
	.vsync_start = VAC + VFP_60HZ,
	.vsync_end = VAC + VFP_60HZ + VSA_R,
	.vtotal = VAC + VFP_60HZ + VSA_R + VBP_R,
};
...
static const struct drm_display_mode performance_mode = {
	...
	.vsync_end = VAC + VFP_90HZ + VSA_R,
	.vtotal = VAC + VFP_90HZ + VSA_R + VBP_R,
};
...
{
	unsigned int bl_lvl;
	char bl_tb0[] = {0x51, 0x0f, 0x0f,0x00};

	bl_lvl = wingtech_bright_to_bl(level,1023,40,4095,48);

	bl_tb0[1] = ((bl_lvl >> 4) & 0xff);
	bl_tb0[2] = (bl_lvl & 0x0f);
...
static int txdsharp_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
...
static int txdsharp_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->prepared = true;
	ctx->enabled = true;
	drm_panel_init(&ctx->panel, dev, &txdsharp_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	ctx->panel.dev = dev;
...
static void txdsharp_remove(struct mipi_dsi_device *dsi)
{
...
static int __init txdsharp_drv_init(void)
{
	int ret = 0;

	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	ret = mipi_dsi_driver_register(&txdsharp_driver);
	if (ret < 0)
...
static void __exit txdsharp_drv_exit(void)
{
	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	mipi_dsi_driver_unregister(&txdsharp_driver);
	mtk_panel_unlock();
	pr_notice("%s-\n", __func__);
}
module_init(txdsharp_drv_init);
module_exit(txdsharp_drv_exit);
