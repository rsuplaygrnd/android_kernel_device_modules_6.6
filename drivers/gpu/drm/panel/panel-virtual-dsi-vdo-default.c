...
static const struct drm_display_mode default_mode = {
	...
	.hsync_end = HAC + HFP_60HZ + HSA_Q,
	.htotal = HAC + HFP_60HZ + HSA_Q + HBP_Q,
	.vdisplay = VAC,
	.vsync_start = VAC + VFP,
	.vsync_end = VAC + VFP + VSA_Q,
	.vtotal = VAC + VFP + VSA_Q + VBP_Q,
};
...
static int virtual_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->prepared = true;
	ctx->enabled = true;
	drm_panel_init(&ctx->panel, dev, &virtual_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	ctx->panel.dev = dev;
...
static void virtual_remove(struct mipi_dsi_device *dsi)
{
...
static int __init virtual_drv_init(void)
{
	...
	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	ret = mipi_dsi_driver_register(&virtual_driver);
	if (ret < 0)
...
static void __exit virtual_drv_exit(void)
{
	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	mipi_dsi_driver_unregister(&virtual_driver);
	mtk_panel_unlock();
	pr_notice("%s-\n", __func__);
}
