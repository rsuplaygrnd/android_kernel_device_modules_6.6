...
static int tianma_unprepare(struct drm_panel *panel)
{
	...
	if(w2_gesture_status == 1 ){
		printk("[ILITEK]tianma_unprepare w2_gesture_status = 1\n ");
	} else {
		printk("[ILITEK]tianma_unprepare w2_gesture_status = 0\n ");
#if 0
		ctx->reset_gpio = devm_gpiod_get(ctx->dev, "reset", GPIOD_OUT_HIGH);
		gpiod_set_value(ctx->reset_gpio, 0);
		devm_gpiod_put(ctx->dev, ctx->reset_gpio);
...
static const struct drm_display_mode default_mode = {
	...
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA_A,
	.htotal = HAC + HFP + HSA_A + HBP_A,
	.vdisplay = VAC,
	.vsync_start = VAC + VFP_60HZ,
	.vsync_end = VAC + VFP_60HZ + VSA_A,
	.vtotal = VAC + VFP_60HZ + VSA_A + VBP_A,
};
...
static const struct drm_display_mode performance_mode = {
	...
	.vsync_end = VAC + VFP_90HZ + VSA_A,
	.vtotal = VAC + VFP_90HZ + VSA_A + VBP_A,
};
				 ...
				 unsigned int level)
{
	...
	bl_lvl = wingtech_bright_to_bl(level,1023,40,2047,24);
