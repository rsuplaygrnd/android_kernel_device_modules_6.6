...
static int djn_unprepare(struct drm_panel *panel)
{
	...
	if (w2_gesture_status == 1) {
		printk("[HXTP]djn_unprepare w2_gesture_status = 1\n ");
	} else {
		printk("[HXTP]djn_unprepare w2_gesture_status = 0\n ");
...
static const struct drm_display_mode default_mode = {
	...
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA_D,
	.htotal = HAC + HFP + HSA_D + HBP_D,
	.vdisplay = VAC,
	.vsync_start = VAC + VFP_60HZ,
	.vsync_end = VAC + VFP_60HZ + VSA_D,
	.vtotal = VAC + VFP_60HZ + VSA_D + VBP_D,
};
...
static const struct drm_display_mode performance_mode = {
	...
	.vsync_end = VAC + VFP_90HZ + VSA_D,
	.vtotal = VAC + VFP_90HZ + VSA_D + VBP_D,
};
...
static void djn_panel_shutdown(struct mipi_dsi_device *dsi)
{
	...
	w2_gesture_status = 0;
	pr_info("optimize shutdown sequence w2_gesture_status is 0 %s++\n", __func__);
}
