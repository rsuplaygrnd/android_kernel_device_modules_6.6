...
static int chuangwei_unprepare(struct drm_panel *panel)
{
	...
	msleep(200);
	if(w2_gesture_status == 1){
		printk("[ILITEK]chuangwei_unprepare w2_gesture_status = 1\n ");
	} else {
		printk("[ILITEK]chuangwei_unprepare w2_gesture_status = 0\n ");
#if 0
		ctx->reset_gpio = devm_gpiod_get(ctx->dev, "reset", GPIOD_OUT_HIGH);
		gpiod_set_value(ctx->reset_gpio, 0);
		devm_gpiod_put(ctx->dev, ctx->reset_gpio);
...
static const struct drm_display_mode default_mode = {
	...
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	.hsync_end = HAC + HFP + HSA_C,
	.htotal = HAC + HFP + HSA_C + HBP_C,
	.vdisplay = VAC,
	.vsync_start = VAC + VFP_60HZ,
	.vsync_end = VAC + VFP_60HZ + VSA_C,
	.vtotal = VAC + VFP_60HZ + VSA_C + VBP_C,
};
...
static const struct drm_display_mode performance_mode = {
	...
	.vsync_end = VAC + VFP_90HZ + VSA_C,
	.vtotal = VAC + VFP_90HZ + VSA_C + VBP_C,
};
				 ...
				 unsigned int level)
{
	...
	bl_lvl = wingtech_bright_to_bl(level,1023,40,4095,48);

	bl_tb0[1] = ((bl_lvl >> 8) & 0xf);
	bl_tb0[2] = (bl_lvl & 0xff);
	pr_info("%s in-backlight = %d,out-backlight = %d,bl_tb0[1] = %x,bl_tb0[2] = %x\n", __func__, level,bl_lvl,bl_tb0[1],bl_tb0[2]);
...
static int chuangwei_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	//connector->display_info.width_mm = (68430/1000);
	//connector->display_info.height_mm = (152570/1000);
...
static int chuangwei_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->prepared = true;
	ctx->enabled = true;
	drm_panel_init(&ctx->panel, dev, &chuangwei_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	ctx->panel.dev = dev;
...
static void chuangwei_remove(struct mipi_dsi_device *dsi)
{
...
static int __init chuangwei_drv_init(void)
{
	int ret = 0;

	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	ret = mipi_dsi_driver_register(&chuangwei_driver);
	if (ret < 0)
...
static void __exit chuangwei_drv_exit(void)
{
	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	mipi_dsi_driver_unregister(&chuangwei_driver);
	mtk_panel_unlock();
	pr_notice("%s-\n", __func__);
}
module_init(chuangwei_drv_init);
module_exit(chuangwei_drv_exit);
