...
static u32 adjust_moisture_setting(u32 moistureID, u32 eintID)
{
	...
	pr_notice("%s moistureID:%d accdet_thing_in_flag:%d cur_eint_state:%d\n",
			__func__, moistureID, accdet->thing_in_flag, accdet->cur_eint_state);
...
static void recover_moisture_analog_setting(void)
{
	if (accdet_dts.moisture_detect_mode == 0x1) {
		...
		accdet_clear_bit(RG_EINT0HIRENB_ADDR, RG_EINT0HIRENB_SFT);
		/* enable mtest en */
...
#if IS_ENABLED(CONFIG_WT_HP_MISDETECTION)
	return;
#endif
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	mutex_lock(&accdet->res_lock);
	if (accdet->eint_sync_flag) {
		if ((pre_cable_type != accdet->cable_type) && (accdet->cable_type == NO_DEVICE)) {
			pr_info("%s() Headset has been plugout.set state here\n",__func__);
			accdet->cable_type = pre_cable_type;
			headset_plug_out();
		} else if ((pre_cable_type != accdet->cable_type) ||
			(accdet->cable_type == HEADSET_MIC))
			send_status_event(accdet->cable_type, 1);
	}
	mutex_unlock(&accdet->res_lock);
...
static u32 config_moisture_detect_1_0(void)
{
	...
	if (accdet_dts.eint_use_ext_res == 0x1) {
		...
		pr_notice("[ACCDET Debug] bypass internal series R (0x%x)=0x%x\n",
			RG_ACCDETSPARE_ADDR,
			accdet_read(RG_ACCDETSPARE_ADDR));
	}
	return 0;
}
...
static int accdet_get_dts_data(void)
{
	...
	ret = of_property_read_u32(node, "no-use-comparator",
	...
	ret = of_property_read_u32(node, "moisture-vm",
		&accdet->moisture_vm);
	if (ret) {
